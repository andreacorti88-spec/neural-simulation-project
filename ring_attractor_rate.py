...
NOTA (sezione 4.4.3 del resoconto): questo file era stato accidentalmente
sovrascritto con una variante diversa (kernel a cappello messicano, F non
saturante) che non riproduceva piu' i risultati sopra -- un vero bug di
riproducibilita', scoperto durante la diagnosi formale del bias di
posizione (sezione 4.4.3) confrontando l'output di questo script con il
codice storico incorporato nel resoconto stesso. Ripristinato qui alla
versione che genera davvero i risultati documentati in 4.4.2.
"""
...
if __name__ == '__main__':
    ...
    for c in centers:
        ...
        bias = ((pos - c + 0.5) % 1.0) - 0.5
        print(f"  Stimolo a x={c:.2f} -> bump finale a x={pos:.3f} "
              f"(picco={r_c.max():.3f}, bias={bias:+.3f})")
    ...
    np.random.seed(3)
    r_perturbed = r_stable + np.random.normal(0, 0.3, N)
    r_recovered, history_recovery = run_simulation(
        stim_center=0.25, apply_stim=False, r_init=r_perturbed,
        n_steps=int(200/dt))
    pos_before = x[np.argmax(r_stable)]
    pos_after = x[np.argmax(r_recovered)]
    print(f"  Posizione del bump prima della perturbazione: x={pos_before:.3f}")
    print(f"  Posizione del bump 200ms dopo la perturbazione: x={pos_after:.3f}")
    if abs(pos_after - pos_before) < 0.02:
        print("  >>> IL BUMP TORNA ALLA SUA POSIZIONE ORIGINALE. "
              "Vero attrattore confermato. <<<")

    # -------------------------------------------------------
    # Figura riassuntiva (persistenza + multistabilita')
    # -------------------------------------------------------
    fig, axes = plt.subplots(1, 2, figsize=(11, 4.2))

    axes[0].imshow(history.T, aspect='auto', origin='lower', cmap='inferno',
                    extent=[0, T_total, 0, 1])
    axes[0].axvline(stim_duration_ms, color='cyan', linestyle='--',
                     linewidth=1, label='fine stimolo')
    axes[0].set_xlabel('Tempo (ms)')
    axes[0].set_ylabel('Posizione sull\'anello')
    axes[0].set_title('Persistenza del bump dopo rimozione dello stimolo')
    axes[0].legend(fontsize=8)

    for c, pos in zip(centers, final_positions):
        axes[1].scatter([c], [pos], s=60)
    axes[1].plot([0, 1], [0, 1], '--', color='gray', linewidth=0.8,
                 label='nessun bias (y=x)')
    axes[1].set_xlabel('Posizione dello stimolo')
    axes[1].set_ylabel('Posizione finale del bump')
    axes[1].set_title('Multistabilita\': 5 stimoli, 5 attrattori')
    axes[1].legend(fontsize=8)
    axes[1].grid(alpha=0.3)

    plt.tight_layout()
    plt.savefig('ring_attractor_rate.png', dpi=150)
    print("\nGrafico salvato in ring_attractor_rate.png")
