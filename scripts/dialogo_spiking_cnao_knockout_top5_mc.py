"""
Sezione 5.38: la scelta di "esattamente 10 neuroni" per il knockout,
usata in ogni esperimento dalle sezioni 5.22 in poi, era arbitraria --
una soglia relativa (top-10%), non una soglia assoluta di danno
biologico (limite gia' dichiarato nel README di geant4_cnao_ring). Il
meccanismo (l'effetto scala con lo sbilanciamento fisico tra le due
zone di codifica) generalizza a dimensioni di knockout diverse, o
"10" era in qualche modo speciale?

Qui: TOP-5 (i 5 neuroni a dose media piu' alta sui 4 seed della 5.32,
carbonio-12 al picco). Overlap con le zone di codifica: ZERO in
entrambe -- nessuno dei 5 neuroni piu' colpiti cade nella zona di
x=0.15 ne' in quella di x=0.65. Previsione dal meccanismo gia'
stabilito: effetto nullo, indistinguibile dal controllo negativo
(come il caso ~1-vs-1 della 5.27 e lo 0-vs-0 della 5.24).
"""

KNOCKOUT = sorted([7, 29, 37, 73, 82])  # top-5 per DOSE MEDIA sui 4 seed
                                          # 111/222/333/444 (carbonio-12
                                          # 120 MeV/u, picco 34.5mm --
                                          # overlap 0-vs-0, vedi 5.38)
...
tight_layout()
savefig('dialogo_spiking_cnao_knockout_top5.png', dpi=150)
print("\nGrafico salvato in dialogo_spiking_cnao_knockout_top5.png")
