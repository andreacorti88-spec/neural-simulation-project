"""
Domanda clinicamente rilevante, mai posta finora nelle sezioni 5.22-
5.28: dopo il knockout, la rete puo' RECUPERARE allenando piu' a lungo?
Il danno da radiazione e' strutturalmente permanente (i neuroni spenti
restano spenti, come nella realta' biologica -- non si rigenerano), ma
la domanda e' se la plasticita' RESIDUA (le sinapsi dei neuroni ancora
sani) possa riorganizzarsi nel tempo per compensare, instradando il
messaggio attraverso un percorso alternativo -- la stessa logica della
plasticita' compensatoria osservata clinicamente dopo lesioni cerebrali
localizzate.

Usa il set di neuroni spenti con l'effetto piu' forte finora osservato
(sezione 5.28, protone 70 MeV al picco di Bragg: 4 neuroni in zona
x=0.65, zero in zona x=0.65 -- rapporto 7.07x su 60 ripetizioni).
Allenamento esteso di 5x (300 ripetizioni invece di 60), con
valutazione della qualita' del messaggio x=0.65 a intervalli regolari
lungo tutto il training, non solo a inizio/fine -- per vedere una vera
traiettoria di recupero (o la sua assenza), non solo un singolo
confronto prima/dopo.
"""

KNOCKOUT = sorted([31, 33, 47, 54, 60, 63, 65, 66, 94, 95])  # top-10 dose,
...
def costruisci_anello(E, I_pop, seed, knockout=()):
    ...
    rng_ei = np.random.RandomState(seed + 100 + SEED*17)
...
syn_A = costruisci_anello(A_E, A_I, seed=1000+SEED, knockout=KNOCKOUT)
syn_B = costruisci_anello(B_E, B_I, seed=2000+SEED, knockout=KNOCKOUT)
...
N_RIPETIZIONI_TOT = 300  # esteso 5x (era 60) per vedere una vera
                           # traiettoria di recupero, non solo inizio/fine
T_STIM = 60*ms
T_PAUSA = 340*ms
...
tight_layout()
savefig('dialogo_spiking_cnao_recupero.png', dpi=150)
print("\nGrafico salvato in dialogo_spiking_cnao_recupero.png")
...
# --- Traiettoria di recupero: qualita' di ciascun messaggio in finestre
# consecutive di 10 ripetizioni lungo TUTTO il training, non solo
# inizio/fine -- per vedere se x=0.65 (danneggiato dal knockout) mostra
# un miglioramento progressivo (recupero) o resta stabilmente basso
# (danno strutturale permanente, non compensabile con piu' training) ---
FINESTRA = 10
n_finestre = N_RIPETIZIONI_TOT // (FINESTRA * len(MESSAGGI))


def valuta_finestra(k, indice_finestra):
    tempi_k = [t0 for (kk, t0) in storico if kk == k]
    inizio = indice_finestra * FINESTRA
    fine = inizio + FINESTRA
    lista = tempi_k[inizio:fine]
    n_tot, posizioni = 0, []
    for t0 in lista:
        fin = t0 + ciclo_ms
        m = mask_b & (spikes.t/ms >= t0) & (spikes.t/ms < fin)
...
traiettoria = {k: {'n': [], 'err': []} for k in range(len(MESSAGGI))}
for k, m in enumerate(MESSAGGI):
    for fw in range(n_finestre):
        n_tot, centro = valuta_finestra(k, fw)
        err = dist_circ(centro - m) if centro is not None else None
        traiettoria[k]['n'].append(n_tot)
        traiettoria[k]['err'].append(err)

print("\n" + "="*70)
print(f"TRAIETTORIA DI RECUPERO (finestre di {FINESTRA} ripetizioni ciascuna, "
      f"{n_finestre} finestre totali per messaggio)")
print("="*70)
for k, m in enumerate(MESSAGGI):
    stringa = ", ".join(str(n) for n in traiettoria[k]['n'])
    print(f"  x={m}: spike per finestra: [{stringa}]")

fig2 = figure(figsize=(11, 4))
colori_msg = ['C0', 'C3']
for k, m in enumerate(MESSAGGI):
    plot(range(n_finestre), traiettoria[k]['n'], 'o-', color=colori_msg[k],
         label=f'x={m}' + (' (danneggiato dal knockout)' if m == 0.65 else ''))
xlabel(f'Finestra di training ({FINESTRA} ripetizioni ciascuna)')
ylabel('Spike di B nella finestra')
title('Traiettoria di recupero durante il training esteso (300 ripetizioni)')
legend(fontsize=9)
tight_layout()
savefig('dialogo_spiking_cnao_recupero_traiettoria.png', dpi=150)
print("\nGrafico traiettoria salvato in dialogo_spiking_cnao_recupero_traiettoria.png")

# confronto quantitativo: prima meta' del training esteso vs seconda meta'
meta = n_finestre // 2
for k, m in enumerate(MESSAGGI):
    prima_meta = np.mean(traiettoria[k]['n'][:meta])
    seconda_meta = np.mean(traiettoria[k]['n'][meta:])
    print(f"  x={m}: prima meta' training={prima_meta:.1f} spike/finestra, "
          f"seconda meta'={seconda_meta:.1f} spike/finestra, "
          f"variazione={'+' if seconda_meta>=prima_meta else ''}{(seconda_meta-prima_meta)/max(prima_meta,0.01)*100:.0f}%")

print(f"MC_RIGA seed={SEED} x065_prima={np.mean(traiettoria[1]['n'][:meta]):.1f} x065_seconda={np.mean(traiettoria[1]['n'][meta:]):.1f} x015_prima={np.mean(traiettoria[0]['n'][:meta]):.1f} x015_seconda={np.mean(traiettoria[0]['n'][meta:]):.1f}")
