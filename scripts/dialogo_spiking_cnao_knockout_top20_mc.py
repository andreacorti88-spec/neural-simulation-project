"""
Sezione 5.38 (vedi anche la variante TOP-5, stesso giorno): test di
confine sulla dimensione del knockout, non piu' fissa a 10 neuroni.

Qui: TOP-20 (i 20 neuroni a dose media piu' alta sui 4 seed della
5.32, carbonio-12 al picco). Overlap con le zone di codifica: 3
neuroni (12, 18, 20) in x=0.15, 1 neurone (70, lo stesso della 5.33)
in x=0.65 -- uno sbilanciamento di segno OPPOSTO al caso TOP-10 (che
aveva 0 in x=0.15 e 1 in x=0.65). Previsione, deliberatamente
falsificabile nello stesso spirito della 5.25/5.26: se il meccanismo e'
davvero guidato dallo sbilanciamento fisico e non da un artefatto
legato a "esattamente 10 neuroni", l'effetto dovrebbe INVERTIRSI --
questa volta x=0.15 piu' danneggiato di x=0.65, con una magnitudine
paragonabile al 3-contro-1 della sezione 5.26 (che diede 1.81x).
"""

KNOCKOUT = sorted([0, 5, 7, 12, 18, 20, 26, 28, 29, 32, 37, 41, 45, 47,
                    48, 70, 73, 79, 82, 99])  # top-20 per DOSE MEDIA sui
                                                # 4 seed 111/222/333/444
...
tight_layout()
savefig('dialogo_spiking_cnao_knockout_top20.png', dpi=150)
print("\nGrafico salvato in dialogo_spiking_cnao_knockout_top20.png")
