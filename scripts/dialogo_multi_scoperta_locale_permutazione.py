"""
Estensione di dialogo_multi_scoperta_locale_montecarlo.py: la verifica su 8
seed indipendenti aveva dato 91.7% di successo aggregato (robusto), ma con
un pattern inatteso sul turno 5 -- il messaggio in POSIZIONE 1 del
round-robin (x=0.10) e' risultato perfetto in tutti gli 8 seed, mentre il
piu' fragile non era l'ultimo (x=0.70, 75%) come atteso ma il SECONDO
(x=0.40, solo 37.5%). Domanda aperta: questa fragilita' segue il VALORE
del messaggio (0.40 resta fragile ovunque venga messo) o la POSIZIONE nel
round-robin (chi va per secondo e' fragile, chiunque esso sia)?

Qui l'ordine del round-robin viene RUOTATO -- [0.40, 0.70, 0.10] invece di
[0.10, 0.40, 0.70], stesso codice, stessi seed (101-108) -- per
disambiguare le due ipotesi. Se la fragilita' sul turno 5 resta legata al
valore 0.40 (ora in posizione 1), e' una proprieta' del valore. Se si
sposta a chiunque occupi la posizione 2 (ora 0.70), e' un effetto puro
dell'ordine temporale di allenamento.
...
if __name__ == '__main__':
    ...
    # ordine ruotato rispetto a dialogo_multi_scoperta_locale_montecarlo.py
    # ([0.10, 0.40, 0.70]): serve a distinguere se la fragilita' osservata
    # sul turno 5 (37.5% per il messaggio in posizione 2 su 8 seed) segue
    # il VALORE del messaggio (0.40 resta fragile ovunque si trovi) o la
    # POSIZIONE nel round-robin (chi va per secondo e' fragile, chiunque
    # esso sia)
    MESSAGGI = [0.40, 0.70, 0.10]
    N_EPISODES = 13500
    EVAL_EVERY = 500
    N_TRIAL_EVAL = 15
    ...
    for m in MESSAGGI:
        ...
        # 'pos' = posizione nel round-robin (0=primo, 1=secondo, 2=terzo)
        print(f"MC_RIGA seed={SEED} x={m:.2f} pos={MESSAGGI.index(m)} B1={dopo['tasso_B1']*100:.0f} "
    ...
    plt.tight_layout()
    plt.savefig(f'dialogo_multi_scoperta_locale_permutazione_seed{SEED}.png', dpi=150)
    print(f"\nGrafico salvato in dialogo_multi_scoperta_locale_permutazione_seed{SEED}.png")
