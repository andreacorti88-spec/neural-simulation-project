"""
Verifica MC (4 seed) del test di confine della sezione 5.30: la 5.29 ha
mostrato che dopo un knockout parziale (10 neuroni a dose piu' alta, di
cui solo 4 nella zona di codifica) il messaggio danneggiato recupera
progressivamente con training esteso. Qui il danno e' portato al limite
--  spenti TUTTI E 11 i neuroni della zona di codifica di x=0.65 (l'intero
gruppo_stim) -- per vedere se il meccanismo di recupero regge anche in
questo caso estremo, o se esiste un punto di non ritorno oltre il quale
la plasticita' residua non ha piu' nessun percorso alternativo da
sfruttare. Sul seed di partenza (singolo run) il recupero e' sparito
del tutto: verifica qui su 4 seed indipendenti.
"""

KNOCKOUT = sorted([60, 61, 62, 63, 64, 65, 66, 67, 68, 69, 70])  # LESIONE
# COMPLETA: tutti gli 11 neuroni della zona di codifica di x=0.65 (non
# piu' i 10 a dose piu' alta di uno scenario CNAO -- un test di confine,
# non un dato fisico)
...
tight_layout()
savefig('dialogo_spiking_lesione_completa_mc.png', dpi=150)
print("\nGrafico salvato in dialogo_spiking_lesione_completa_mc.png")
...
legend(fontsize=9)
tight_layout()
savefig('dialogo_spiking_lesione_completa_mc_traiettoria.png', dpi=150)
print("\nGrafico traiettoria salvato in dialogo_spiking_lesione_completa_mc_traiettoria.png")
