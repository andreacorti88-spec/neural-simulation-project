"""
Tredicesimo tentativo, ultimo passo del filone bidirezionale spiking:
estendere l'architettura completa che ha risolto la sezione 5.17
(interruttore di trasmissione + normalizzazione omeostatica graduale +
soppressione diretta di emergenza, 8/8 seed stabili con crescita
bidirezionale) a PIU' MESSAGGI, esattamente come il modello a rate aveva
fatto il suo passo finale (dialogo_multi_scoperta_bidirezionale.py,
sezione 5.5.13) estendendo il canale bidirezionale a turni gia' risolto
(5.5.3) a piu' messaggi.

Stesse due posizioni usate in dialogo_spiking_multi.py (sezione 5.10,
comunicazione unidirezionale multi-messaggio, gia' verificata robusta su
8 seed in 5.11): x=0.15 e x=0.65. Schema a turni esteso a 4 combinazioni
(messaggio, direzione) invece delle 2 originarie (solo direzione),
cicliche in ordine fisso: (m0,A), (m0,B), (m1,A), (m1,B), ripetuto.
"""
...
MESSAGGI = [0.15, 0.65]  # stesse posizioni di dialogo_spiking_multi.py (5.10)
...
N_CICLI = 80  # 20 ripetizioni per ciascuna delle 4 combinazioni (messaggio, direzione)
T_STIM = 60*ms
T_PAUSA = 940*ms
...
tight_layout()
savefig('dialogo_spiking_bidirezionale_multi.png', dpi=150)
print("\nGrafico salvato in dialogo_spiking_bidirezionale_multi.png")
