...
{
  G4cout << " Usage: " << G4endl;
  G4cout << " molecular [-m macro ] [-t nThreads] [-p PhysicsList] [-v vis] [-s seed]" << G4endl;
  ...
  G4cout << "   -s explicit random seed (default: fixed CLHEP default -- the"
         << " original code never called setTheSeed, so every run without"
         << " -s reproduces the SAME sequence; pass -s for an independent"
         << " Monte Carlo realization)." << G4endl;
}
}  // namespace
...
int main(int argc, char** argv)
{
  ...
  for (G4int ii = 1; ii < argc; ii = ii + 2) {
    ...
    else if (G4String(argv[ii]) == "-s") {
      seed = std::stol(argv[ii + 1]);
    }
    else {
      PrintUsage();
      return 1;
  ...
  // Sezione 5.42: il codice originale non chiamava mai setTheSeed, quindi
  // ogni run senza -s riproduce la stessa identica sequenza (seed di
  // default fisso di CLHEP) -- utile per la riproducibilita', ma non da'
  // realizzazioni Monte Carlo indipendenti per una verifica multi-seed.
  if (seed >= 0) {
