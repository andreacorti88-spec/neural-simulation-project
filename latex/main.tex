...
\textit{\textbf{Codice sorgente: due\_\allowbreak{}neuroni.py}}
...
\textit{\textbf{Codice sorgente: esplorazione\_\allowbreak{}parametri.py}}
...
\textit{\textbf{Codice sorgente: plasticita\_\allowbreak{}sinaptica.py}}
...
\textit{\textbf{Codice sorgente: rete\_\allowbreak{}catena.py}}
...
\textit{\textbf{Codice sorgente: rete\_\allowbreak{}convergente.py}}
...
\textit{\textbf{Codice sorgente: classificazione\_\allowbreak{}stdp.py}}
...
\textit{\textbf{Codice sorgente: rete\_\allowbreak{}300\_\allowbreak{}neuroni.py}}
...
\textit{\textbf{Codice sorgente: rete\_\allowbreak{}300\_\allowbreak{}stimolo.py}}
...
\textit{\textbf{Codice sorgente: due\_\allowbreak{}moduli.py}}
...
\textit{\textbf{Codice sorgente: psth\_\allowbreak{}moduli.py}}
...
\emph{Connettivita\textquotesingle{} a probabilita\textquotesingle{} fissa: N\_\allowbreak{}sinapsi $\approx$ p $\times$ N\textsuperscript{2} (impraticabile oltre N\textasciitilde10\textsuperscript{4})}

\emph{Connettivita\textquotesingle{} a grado fisso: N\_\allowbreak{}sinapsi $\approx$ K $\times$ N (praticabile fino a N\textasciitilde10\textsuperscript{7}-10\textsuperscript{8})}
...
\textit{\textbf{Codice sorgente: scaling\_\allowbreak{}fase4.py (ultima delle 4 fasi di scaling automatico)}}
...
\textit{\textbf{Codice sorgente: rete\_\allowbreak{}40M\_\allowbreak{}finale.py}}
...
\emph{P(connessione) = p\_\allowbreak{}locale $\times$ exp(-distanza / $\xi$) + p\_\allowbreak{}scorciatoia}

dove $\xi$ (xi) e\textquotesingle{} la "lunghezza caratteristica" che determina quanto rapidamente la probabilita\textquotesingle{} decresce allontanandosi, e p\_\allowbreak{}scorciatoia e\textquotesingle{} una piccola probabilita\textquotesingle{} costante, indipendente dalla distanza, che rappresenta le connessioni a lungo raggio.
...
\textit{\textbf{Codice sorgente: rete\_\allowbreak{}smallworld.py}}
...
\emph{$\tau$ $\cdot$ dr/dt = -r + F( J*r/N - inibizione\_\allowbreak{}globale$\cdot$media(r) + I\_\allowbreak{}esterno )}
...
\item
  La soluzione finale, funzionante: separare esplicitamente il termine di eccitazione (solo locale, kernel Gaussiano stretto, nessuna componente inibitoria nello stesso kernel) dal termine di inibizione (reso esplicitamente GLOBALE, proporzionale alla media dell\textquotesingle intera rete, sottratto separatamente), combinato con una funzione di attivazione rettificata che SATURA a un valore massimo esplicito (F(u) = clip(u, 0, r\_\allowbreak{}max) invece di una sigmoide morbida o di una rettificazione senza limite). Questa combinazione -\/- eccitazione locale forte, inibizione globale per la competizione, saturazione hard per la stabilita\textquotesingle{} -\/- e\textquotesingle{} risultata quella che produce un vero bump localizzato e persistente
\end{enumerate}
...
\textit{\textbf{Codice sorgente: ring\_\allowbreak{}attractor\_\allowbreak{}rate.py (versione finale verificata)}}

\lstinputlisting[language=Python]{scripts/ring_attractor_rate.py}
\subsubsection{4.4.3 Diagnosi formale del bias di posizione: non e\textquotesingle{} un artefatto di discretizzazione}\label{diagnosi-formale-del-bias-di-posizione-non-e-un-artefatto-di-discretizzazione}

La sezione 4.4.2 aveva osservato un bias sistematico di circa 0.045 unita\textquotesingle{} tra la posizione dello stimolo e quella del bump risultante, ipotizzando -\/- senza verificarlo -\/- che fosse "verosimilmente un artefatto di discretizzazione della griglia spaziale a N=200 punti". La sezione 9 lo elencava esplicitamente come domanda aperta: "determinare se sia un artefatto puramente numerico o rifletta una proprieta\textquotesingle{} reale (per quanto non desiderata) del modello discretizzato". Questa sezione risponde con tre test indipendenti e falsificabili.

Un bug di riproducibilita\textquotesingle{} scoperto durante la preparazione di questa diagnosi, corretto prima di procedere: rieseguendo ring\_\allowbreak{}attractor\_\allowbreak{}rate.py per confrontarne l\textquotesingle output con quanto documentato in 4.4.2, il file al livello principale del repository risultava sovrascritto con un\textquotesingle implementazione diversa da quella realmente validata -\/- un kernel a "cappello messicano" (due Gaussiane opposte, eccitazione meno inibizione locale) con una funzione di attivazione NON saturante, invece del kernel eccitatorio singolo piu\textquotesingle{} inibizione GLOBALE (proporzionale alla media dell\textquotesingle intera rete) con attivazione rettificata satura a r\_\allowbreak{}max=5.0 effettivamente usato per produrre i quattro risultati verificati in 4.4.2. Con questa versione sbagliata, il bump si spegneva sempre entro fine simulazione, mai il comportamento documentato. Confrontando con il codice storico incorporato nel resoconto stesso (la tabella di codice della sezione 4.4.2, congelata al momento della validazione originale), e\textquotesingle{} stato possibile ricostruire e ripristinare la versione realmente funzionante. Rieseguita, la versione corretta riproduce ORA esattamente il bias di -0.045 in tutte e 5 le posizioni di 4.4.2 (picco di saturazione a 5.000 in ognuna) -\/- confermando che il file ripristinato e\textquotesingle{} davvero quello dietro ai risultati documentati, non solo un\textquotesingle ipotesi plausibile.

PARTE 1 -\/- scaling con la risoluzione della griglia N. Se il bias fosse un artefatto della somma di Riemann che approssima l\textquotesingle integrale di convoluzione continuo (rec\_\allowbreak{}input = J\_\allowbreak{}exc @ r / N), dovrebbe ridursi sistematicamente all\textquotesingle aumentare di N, verso 1/N. Lo stimolo e\textquotesingle{} posizionato deliberatamente a x=0.2537, una coordinata che non coincide MAI esattamente con un nodo della griglia per nessuno degli N testati (50, 100, 200, 400, 800, 1600), cosi\textquotesingle{} da non confondere lo scaling reale con un banale aggancio del picco al nodo di griglia piu\textquotesingle{} vicino quando N e\textquotesingle{} piccolo.

Risultato: il bias resta PRATICAMENTE COSTANTE su tutto l\textquotesingle intervallo (-0.034 a N=50, fino a -0.046 a N=1600) -\/- un fattore di variazione di appena 0.74x tra i due estremi, contro il fattore \textasciitilde32x di RIDUZIONE che un vero artefatto di quantizzazione 1/N avrebbe dovuto produrre passando da N=50 a N=1600. Il bias, se qualcosa, cresce leggermente con N invece di sparire -\/- l\textquotesingle esatto opposto della predizione dell\textquotesingle ipotesi originale.

PARTE 2 -\/- dipendenza dalla posizione sub-griglia, a N=200 fissa. Se il bias dipendesse dalla posizione dello stimolo RELATIVA al nodo di griglia piu\textquotesingle{} vicino (la firma diretta di un artefatto di lettura per quantizzazione), farebbe scorrere lo stimolo con continuita\textquotesingle{} attraverso alcune celle di griglia dovrebbe mostrare una struttura periodica di ampiezza paragonabile al bias stesso.

Risultato: emerge davvero una modulazione periodica con periodo ESATTAMENTE 1/N (la firma classica della quantizzazione della lettura del picco tramite argmax su una griglia discreta) -\/- ma di ampiezza molto piu\textquotesingle{} piccola del bias principale (dente di sega tra -0.0415 e -0.046, appena 0.0045 di ampiezza picco-picco, un decimo del bias di \textasciitilde0.044 che l\textquotesingle ipotesi di 4.4.2 cercava di spiegare). Un vero artefatto di discretizzazione esiste, quindi, ma e\textquotesingle{} un effetto secondario e trascurabile sovrapposto a un bias molto piu\textquotesingle{} grande e sostanzialmente indipendente dalla posizione sub-griglia.

PARTE 3 -\/- dipendenza dal passo di integrazione temporale dt, a N=200 fissa. Se il bias fosse un artefatto di troncamento dell\textquotesingle integrazione di Eulero esplicita (l\textquotesingle errore locale di un passo di Eulero scala linearmente con dt), dovrebbe ridursi proporzionalmente al diminuire di dt.

Risultato: il bias e\textquotesingle{} IDENTICO a cinque cifre decimali (-0.04370) per ognuno dei quattro valori di dt testati (0.2, 0.1, 0.05, 0.025 ms, un fattore 8x) -\/- nessuna traccia di dipendenza da dt, contro il fattore \textasciitilde8x di riduzione atteso da un vero artefatto di troncamento lineare in dt.

Conclusione: il bias di posizione del ring attractor NON e\textquotesingle{} un artefatto numerico -\/- ne\textquotesingle{} della discretizzazione spaziale (Parte 1), ne\textquotesingle{} dell\textquotesingle integrazione temporale (Parte 3). E\textquotesingle{} una proprieta\textquotesingle{} REALE, per quanto non desiderata, della dinamica continua del modello. L\textquotesingle ipotesi piu\textquotesingle{} plausibile, non verificata ulteriormente in questo lavoro: un\textquotesingle asimmetria nella fase TRANSITORIA di formazione del bump, dove il termine di inibizione globale (proporzionale alla media istantanea dell\textquotesingle INTERA rete, che cambia rapidamente mentre il bump si forma) potrebbe non preservare esattamente la simmetria di traslazione durante la crescita iniziale, prima che il sistema si assesti nella sua forma stazionaria finale -\/- una proprieta\textquotesingle{} della traiettoria dinamica, non della configurazione di equilibrio in se\textquotesingle. Una modulazione periodica di ampiezza molto piu\textquotesingle{} piccola (un decimo del bias principale), con periodo esattamente 1/N, e\textquotesingle{} presente ed e\textquotesingle{} davvero un artefatto di quantizzazione della lettura del picco -\/- ma e\textquotesingle{} un effetto secondario, non la spiegazione del fenomeno principale ipotizzata in 4.4.2.

\includegraphics[width=6.3in,height=1.7071in]{media/media/image53.png}

\emph{Fig. 52 --- Diagnosi del bias di posizione: (sinistra) bias vs risoluzione della griglia N, praticamente costante contro lo scaling 1/N atteso per un artefatto; (centro) bias vs posizione sub-griglia a N=200, mostra una modulazione periodica secondaria di ampiezza molto minore del bias principale; (destra) bias vs passo di integrazione dt, identico su un fattore 8x.}

\textit{\textbf{Codice sorgente: diagnosi\_\allowbreak{}bias\_\allowbreak{}posizione\_\allowbreak{}scaling.py}}

\lstinputlisting[language=Python]{scripts/diagnosi_bias_posizione_scaling.py}
...
\textit{\textbf{Codice sorgente: chained\_\allowbreak{}attractors.py}}
...
\textit{\textbf{Codice sorgente: competing\_\allowbreak{}targets.py}}
...
\emph{peso\_\allowbreak{}scelto $\leftarrow$ peso\_\allowbreak{}scelto + tasso\_\allowbreak{}apprendimento $\times$ (ricompensa - peso\_\allowbreak{}scelto)}
...
\textit{\textbf{Codice sorgente: rl\_\allowbreak{}bandit.py}}
...
\textit{\textbf{Codice sorgente: contextual\_\allowbreak{}policy.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}ring\_\allowbreak{}attractor.py}}

\lstinputlisting[language=Python]{scripts/dialogo_ring_attractor.py}
\subsubsection{5.5.2 Sinapsi apprese con un rinforzo Hebbiano a grana di turno}\label{sinapsi-apprese-con-un-rinforzo-hebbiano-a-grana-di-turno}

Le due mappe di offset diventano qui due matrici di peso N x N vere e proprie (W\_\allowbreak{}AB e W\_\allowbreak{}BA), inizializzate a un bias innato DELIBERATAMENTE troppo debole per accendere da solo un bump nella rete che ascolta (soglia di accensione verificata per bisezione: tra scala 0.0025, 0\% di successo, e 0.003, 100\% di successo -\/- l\textquotesingle attrattore e\textquotesingle{} fortemente bistabile, non esiste quasi via di mezzo). Un rinforzo Hebbiano a fine turno rafforza la coppia (posizione di chi ha appena parlato, posizione di chi ha appena risposto), ogni volta che l\textquotesingle ascoltatore forma davvero un bump. Il problema dell\textquotesingle uovo e della gallina -\/- senza connessione funzionante non si osserva mai una coppia da rinforzare -\/- e\textquotesingle{} risolto con un impulso esplorativo a posizione casuale durante il solo training: rottura di simmetria casuale, lo stesso meccanismo gia\textquotesingle{} verificato in 5.2 (TEST 1b).

Risultato per il canale A-\textgreater B: il tasso di successo di B passa da 0\% a 100\% in circa 30-40 episodi (curva sigmoidale, non un salto istantaneo), e la mappa appresa converge a un offset di 0.195 -\/- diverso dal bias innato di partenza (0.3), un risultato genuinamente emergente e non imposto.

Risultato per il canale di ritorno B-\textgreater A: NON si consolida in questo script. A ha gia\textquotesingle{} un bump persistente fin dal turno 0 (e\textquotesingle{} un attrattore), quindi B deve SPODESTARE un bump gia\textquotesingle{} a piena potenza invece di accenderne uno da zero -\/- un compito molto piu\textquotesingle{} difficile. Il peso W\_\allowbreak{}BA cresce comunque nel tempo, ma per pura coincidenza statistica (la posizione di A resta quasi sempre la stessa per ragioni proprie, indipendenti da B), non per un\textquotesingle influenza causale reale: resta comportamentalmente inefficace. Un risultato negativo verificato col codice, non nascosto -\/- nello stesso spirito metodologico dichiarato nell\textquotesingle introduzione di questo documento.

\includegraphics[width=5.8in,height=4.35in]{media/media/image22.png}

\emph{Fig. 22 --- Consolidamento della sinapsi A-\textgreater B: curva di apprendimento e mappa dei pesi prima/dopo.}

\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}stdp.py}}

\lstinputlisting[language=Python]{scripts/dialogo_stdp.py}
\subsubsection{5.5.3 Risoluzione dell\textquotesingle asimmetria: adattamento, turni di silenzio, riscaldamento}\label{risoluzione-dellasimmetria-adattamento-turni-di-silenzio-riscaldamento}

Per rompere l\textquotesingle asimmetria del passo precedente si introduce un ADATTAMENTO a soglia di scarica -\/- l\textquotesingle analogo a rate della variabile di recovery \textquotesingle u\textquotesingle{} del modello di Izhikevich usato in tutto il resto del progetto (sezione 1.2): piu\textquotesingle{} a lungo un ring resta acceso, piu\textquotesingle{} cresce una corrente negativa che alla fine lo spegne da solo. Un test dedicato su un ring isolato mostra che la scarica hard-clipped di questo modello rende l\textquotesingle attrattore fortemente bistabile (collasso brusco, non decadimento morbido); il parametro di adattamento e\textquotesingle{} stato scelto apposta cosi\textquotesingle{} che un bump sopravviva un turno pieno, si affievolisca nel successivo, e collassi da solo un turno dopo. Il problema torna quindi ad essere quello gia\textquotesingle{} risolto (accendere da silenzio), non piu\textquotesingle{} spodestare un bump a piena potenza. Uno schema con un turno di SILENZIO tra un ascolto e l\textquotesingle altro si e\textquotesingle{} rivelato necessario: senza quella pausa A tentava di riaccendersi mentre l\textquotesingle adattamento era ancora al culmine e falliva sempre.

Anche con questi due correttivi, in 2000 episodi la sinapsi B-\textgreater A cresceva circa 7 volte piu\textquotesingle{} lento di A-\textgreater B e non arrivava mai all\textquotesingle autosufficienza. Diagnosi: nei primi episodi la posizione di B non si e\textquotesingle{} ancora stabilizzata (dipende da un A-\textgreater B non ancora consolidato), quindi il rinforzo di B-\textgreater A punta inizialmente a un bersaglio mobile e si diluisce invece di concentrarsi. Corretto con una fase di RISCALDAMENTO (solo A-\textgreater B per i primi 400 episodi) e un tasso di apprendimento piu\textquotesingle{} alto per B-\textgreater A.

Risultato finale, con budget di training sufficiente (\textasciitilde2500 episodi): TUTTI e quattro gli scambi verificati arrivano al 100\% di affidabilita\textquotesingle{} in valutazione pulita (pesi congelati, nessun rumore esplorativo) -\/- B risponde al turno 1 e di nuovo al turno 5, A risponde al turno 3 e di nuovo al turno 7. Il turno 5 e il turno 7 sono associazioni di SECONDO ORDINE (la rete deve rispondere alla posizione assunta dall\textquotesingle altra DOPO che questa ha gia\textquotesingle{} risposto una prima volta, non al messaggio originale): si consolidano solo dopo che la prima risposta e\textquotesingle{} gia\textquotesingle{} stabile, quindi richiedono piu\textquotesingle{} episodi e appaiono nella mappa dei pesi finale come rinforzi secondari, piu\textquotesingle{} deboli del picco principale. L\textquotesingle asimmetria di partenza (B-\textgreater A che non imparava affatto) e\textquotesingle{} risolta in questo run, non solo attenuata.

\includegraphics[width=5.8in,height=4.35in]{media/media/image23.png}

\emph{Fig. 23 --- Curve di apprendimento di entrambi i canali, consolidamento delle due sinapsi, mappa W\_\allowbreak{}AB prima/dopo.}

\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}stdp\_\allowbreak{}adattamento.py}}

\lstinputlisting[language=Python]{scripts/dialogo_stdp_adattamento.py}
\subsubsection{5.5.4 Piu\textquotesingle{} messaggi sulla stessa sinapsi: un test di interferenza}\label{piu-messaggi-sulla-stessa-sinapsi-un-test-di-interferenza}

Ultima domanda, il vero test di generalizzazione nello stesso spirito della sezione 5.4 (che estende un singolo contesto a piu\textquotesingle{} contesti): la STESSA coppia di sinapsi W\_\allowbreak{}AB/W\_\allowbreak{}BA, condivisa (non una matrice separata per messaggio), puo\textquotesingle{} imparare a distinguere PIU\textquotesingle{} messaggi diversi senza che si interferiscano a vicenda? Due messaggi ben separati sull\textquotesingle anello (x=0.15 e x=0.65) vengono alternati durante il training.

Risultato dopo 3000 episodi con messaggio scelto casualmente a ogni episodio: nessuna interferenza sul canale di ritorno (B-\textgreater A al 100\% per entrambi i messaggi, sia al turno 3 sia al turno 7), ma il canale A-\textgreater B si consolida pienamente solo per uno dei due messaggi (x=0.65: 100\%; x=0.15: 0\%) -\/- una possibile spiegazione, a quel punto, era una semplice esposizione ineguale durante il sorteggio casuale.

Per verificarlo e\textquotesingle{} rilanciato l\textquotesingle esperimento con i messaggi alternati in ROUND-ROBIN invece che a sorteggio (9000 episodi, esposizione identica garantita per costruzione) -\/- e il risultato ha smentito quella spiegazione. Il canale A-\textgreater B non si e\textquotesingle{} consolidato per entrambi: si e\textquotesingle{} semplicemente invertito quale dei due messaggi vince (ora x=0.15 arriva al 100\% su tutti e quattro gli scambi, incluso il turno 5 che non si era mai consolidato prima; x=0.65 crolla a 0\% sia al turno 1 sia al turno 5). Con esposizione uguale garantita, il vincitore cambia ma il fenomeno resta identico: e\textquotesingle{} un\textquotesingle interferenza competitiva reale sul canale A-\textgreater B -\/- verificata su due run indipendenti, non un artefatto di esposizione ineguale -\/- mentre il canale B-\textgreater A resta completamente esente da interferenza su entrambe le run. La mappa dei pesi finale mostra un solo picco netto (per il messaggio vincente) invece delle due regioni separate che ci si aspetterebbe da un apprendimento senza competizione: un risultato onesto ma non ancora pienamente spiegato, lasciato come domanda aperta (analoga nello spirito al bias di posizione del ring attractor, sezione 9) piuttosto che forzato in una conclusione piu\textquotesingle{} comoda di quella osservata.

\includegraphics[width=5.8in,height=4.35in]{media/media/image25.png}

\emph{Fig. 24 --- Test di interferenza tra due messaggi (round-robin, 9000 episodi): B-\textgreater A pulito al 100\% per entrambi; A-\textgreater B mostra una vittoria competitiva di un solo messaggio, con un solo picco netto nella mappa dei pesi finale.}

\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}multi\_\allowbreak{}messaggio.py}}

\lstinputlisting[language=Python]{scripts/dialogo_multi_messaggio.py}
\subsubsection{5.5.5 Un tentativo di correzione: normalizzazione omeostatica (risultato parziale)}\label{un-tentativo-di-correzione-normalizzazione-omeostatica-risultato-parziale}

L\textquotesingle interferenza osservata in 5.5.4 suggerisce due spiegazioni possibili: o le sinapsi crescono senza limite e un messaggio finisce per monopolizzare la matrice condivisa, oppure il problema e\textquotesingle{} nella dinamica dell\textquotesingle attrattore di B stesso (una volta che un bump forte si e\textquotesingle{} stabilito in una zona, quella zona diventa un bacino di attrazione piu\textquotesingle{} facile da raggiungere anche per input diversi). La prima spiegazione e\textquotesingle{} verificabile direttamente: si aggiunge una NORMALIZZAZIONE OMEOSTATICA a W\_\allowbreak{}AB -\/- il totale dei pesi in ENTRATA su ogni neurone di B (somma di colonna) non puo\textquotesingle{} superare un tetto fisso, calibrato un po\textquotesingle{} sopra il valore raggiunto in un run di riferimento a un solo messaggio (sezione 5.5.3). E\textquotesingle{} un meccanismo biologicamente motivato: i neuroni veri regolano omeostaticamente il totale delle proprie sinapsi in ingresso. Un test di regressione con un solo messaggio conferma che la normalizzazione non interferisce con l\textquotesingle apprendimento quando non c\textquotesingle e\textquotesingle{} competizione (il totale di colonna resta ben sotto il tetto per tutto il training).

Risultato con due messaggi (stessa configurazione della sezione 5.5.4: 9000 episodi, round-robin): la normalizzazione NON ribalta l\textquotesingle esito -\/- x=0.15 vince ancora e arriva al 100\% su tutti e quattro gli scambi, mentre x=0.65 fallisce sulla risposta primaria di B (turno 1). Pero\textquotesingle{} qualcosa di nuovo compare: per la prima volta si osserva un\textquotesingle INSTABILITA\textquotesingle{} TRANSITORIA vera (intorno all\textquotesingle episodio 7000, x=0.65 raggiunge brevemente il 100\% su tutti e quattro gli scambi prima di ricadere a 0\%) -\/- una prova diretta che il tetto sta creando competizione reale, non ignorata. E il messaggio perdente ottiene comunque un miglioramento parziale: la sua risposta di secondo ordine (turno 5) arriva al 100\% in valutazione finale, cosa che non succedeva mai senza normalizzazione. Il picco di W\_\allowbreak{}AB e\textquotesingle{} piu\textquotesingle{} basso che nel run precedente (0.036 contro 0.047), segno che il tetto sta davvero limitando la crescita.

Interpretazione: la normalizzazione cambia la dinamica competitiva ma non cambia l\textquotesingle esito finale. Questo depone a favore della seconda spiegazione, non della prima -\/- il collo di bottiglia sembra essere la dinamica dell\textquotesingle attrattore di B (quale bacino diventa dominante), non semplicemente la capacita\textquotesingle{} sinaptica disponibile. Limitare i pesi rende la competizione visibile senza risolverla. Una verifica piu\textquotesingle{} diretta di questa ipotesi -\/- ad esempio separando la componente di inibizione globale di B per zona invece che sull\textquotesingle intera popolazione -\/- resta un lavoro futuro, non affrontato qui: un risultato onesto ma non ancora completamente spiegato, lasciato come domanda aperta piuttosto che forzato in una soluzione.

\includegraphics[width=5.8in,height=4.35in]{media/media/image26.png}

\emph{Fig. 25 --- Stesso test di interferenza con normalizzazione omeostatica su W\_\allowbreak{}AB: l\textquotesingle esito non cambia, ma compare un\textquotesingle instabilita\textquotesingle{} transitoria (episodio \textasciitilde7000) assente senza normalizzazione.}

\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}multi\_\allowbreak{}normalizzato.py}}

\lstinputlisting[language=Python]{scripts/dialogo_multi_normalizzato.py}
\subsubsection{5.5.6 Correzione riuscita: esplorazione repulsiva}\label{correzione-riuscita-esplorazione-repulsiva}

La normalizzazione (5.5.5) aveva mostrato che limitare la capacita\textquotesingle{} sinaptica non basta: rende la competizione visibile ma non la risolve. Un\textquotesingle ipotesi diversa: forse i due messaggi non competono davvero per una risorsa condivisa, ma la RICERCA casuale (l\textquotesingle impulso esplorativo a posizione uniforme casuale) riscopre per puro caso la stessa zona "facile" dell\textquotesingle anello per entrambi -\/- coerente con il bias di posizione gia\textquotesingle{} documentato per il ring attractor (sezione 9). La correzione proposta qui non tocca la capacita\textquotesingle{} sinaptica ma la RICERCA stessa: l\textquotesingle impulso esplorativo viene pescato con probabilita\textquotesingle{} inversamente proporzionale a quanto una zona e\textquotesingle{} gia\textquotesingle{} occupata da ALTRI messaggi (esclusa la propria riga, per non respingere un messaggio dalla zona che sta ancora cercando di consolidare -\/- un errore evitato esplicitamente dopo un test di regressione dedicato). Un messaggio ancora senza risposta viene cosi\textquotesingle{} spinto verso territorio libero invece che verso quello gia\textquotesingle{} rivendicato da un altro.

Risultato (stessa configurazione delle sezioni 5.5.4-5.5.5: 9000 episodi, round-robin): la mappa dei pesi finale mostra per la prima volta DUE righe distinte e ben separate, una per ciascun messaggio (x=0.15 e x=0.65), ciascuna con la propria zona di risposta in B, senza sovrapposizione visibile. In valutazione pulita, SETTE scambi su otto arrivano al 100\% (entrambi i messaggi al 100\% su B turno 1, A turno 3, A turno 7; x=0.65 anche al 100\% su B turno 5) -\/- resta a 0\% solo il turno 5 di x=0.15, l\textquotesingle associazione piu\textquotesingle{} profonda (terzo ordine) e piu\textquotesingle{} lenta a consolidarsi gia\textquotesingle{} nelle sezioni precedenti. La curva di apprendimento periodica mostra che anche x=0.65 ha sbloccato quel controllo solo all\textquotesingle ULTIMISSIMO checkpoint del training (episodio 8999 su 9000): x=0.15 sembra sulla stessa traiettoria, solo leggermente in ritardo per puro caso, non bloccato da un\textquotesingle interferenza strutturale.

NOTA (corretta dopo verifica diretta, sezione 5.5.7): l\textquotesingle ipotesi che il problema fosse un bias sistematico di posizione del ring attractor -\/- pochi punti "facili" dell\textquotesingle anello ritrovati per caso da messaggi diversi -\/- e\textquotesingle{} stata poi testata direttamente allenando piu\textquotesingle{} messaggi in modo completamente indipendente (nessuna sinapsi condivisa) e verificando se le posizioni di risposta si raggruppassero. Non l\textquotesingle hanno fatto (sezione 5.5.7): risultano sparse in modo compatibile con una distribuzione uniforme. L\textquotesingle ipotesi del bias di posizione va quindi ABBANDONATA come spiegazione di questo fenomeno. La correzione (esplorazione repulsiva) resta comunque efficace per il motivo corretto, piu\textquotesingle{} semplice: quando i messaggi condividono la STESSA matrice durante il training concorrente, la repulsione basata sull\textquotesingle occupazione ATTUALE di quella matrice (indipendentemente dalla causa dell\textquotesingle occupazione) impedisce comunque le collisioni, senza bisogno che esista un bias di posizione indipendente a monte.

\includegraphics[width=5.8in,height=4.35in]{media/media/image27.png}

\emph{Fig. 26 --- Con esplorazione repulsiva: due righe distinte e separate nella mappa dei pesi, 7 scambi su 8 al 100\%.}

\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}multi\_\allowbreak{}repulsione.py}}

\lstinputlisting[language=Python]{scripts/dialogo_multi_repulsione.py}
\subsubsection{5.5.7 Verifica diretta dell\textquotesingle ipotesi del bias di posizione (smentita)}\label{verifica-diretta-dellipotesi-del-bias-di-posizione-smentita}

La sezione 5.5.6 aveva spiegato il successo della repulsione con l\textquotesingle ipotesi che la ricerca casuale, senza memoria, riscoprisse per puro caso la stessa zona "facile" dell\textquotesingle anello per messaggi diversi -\/- collegandola al bias di posizione gia\textquotesingle{} documentato per il ring attractor (sezione 9). Un\textquotesingle ipotesi del genere si puo\textquotesingle{} verificare direttamente, non solo argomentare: si allenano molti messaggi diversi in modo COMPLETAMENTE INDIPENDENTE (una rete fresca per ciascuno, nessun peso condiviso, nessuna repulsione -\/- l\textquotesingle unica cosa in comune e\textquotesingle{} la fisica del ring attractor) e si controlla se le posizioni di risposta finali si raggruppano.

Risultato con 6 messaggi indipendenti, spaziati uniformemente sull\textquotesingle anello (0.05, 0.20, 0.35, 0.50, 0.65, 0.80): le posizioni target apprese (0.360, 0.185, 0.345, 0.680, 0.965, 0.130) risultano SPARSE, non raggruppate. La distanza circolare media tra coppie di posizioni (0.260) coincide quasi esattamente con quella attesa per punti distribuiti uniformemente e indipendenti sull\textquotesingle anello (\textasciitilde0.26). Nessuna evidenza di clustering in questo campione.

L\textquotesingle ipotesi del bias di posizione come causa dell\textquotesingle interferenza va quindi ABBANDONATA -\/- e il paragrafo corrispondente in 5.5.6 e\textquotesingle{} stato corretto di conseguenza. Questo non invalida la correzione (l\textquotesingle esplorazione repulsiva resta efficace, verificato empiricamente), ma ne cambia la spiegazione corretta: la repulsione funziona perche\textquotesingle{} contrasta la collisione REALE che nasce quando due processi di ricerca condividono la STESSA matrice durante il training CONCORRENTE -\/- non perche\textquotesingle{} esista un bias di posizione indipendente a monte. E\textquotesingle{} un utile promemoria metodologico, nello stesso spirito dichiarato nell\textquotesingle introduzione di questo documento: una correzione puo\textquotesingle{} funzionare per un motivo diverso da quello con cui viene proposta, e l\textquotesingle unico modo per saperlo e\textquotesingle{} testare la spiegazione stessa, non solo il suo effetto.

\includegraphics[width=5.8in,height=5.8in]{media/media/image28.png}

\emph{Fig. 27 --- Sei messaggi allenati in modo indipendente: le posizioni target (rosso) non si raggruppano attorno al messaggio (blu) ne\textquotesingle{} tra loro.}

\textit{\textbf{Codice sorgente: diagnosi\_\allowbreak{}bias\_\allowbreak{}posizione.py}}

\lstinputlisting[language=Python]{scripts/diagnosi_bias_posizione.py}
\subsubsection{5.5.8 Scalabilita\textquotesingle{} a 3 messaggi: separazione migliore, consolidamento peggiore}\label{scalabilita-a-3-messaggi-separazione-migliore-consolidamento-peggiore}

Domanda di scalabilita\textquotesingle{} diretta: la correzione che ha funzionato per 2 messaggi (5.5.6) regge anche con 3? Stesso meccanismo, nessuna modifica all\textquotesingle algoritmo, solo il budget di training scalato in proporzione (WARMUP e N\_\allowbreak{}EPISODES triplicati per dare a ciascun messaggio lo stesso numero di episodi effettivi della versione a 2).

Primo tentativo (REPULSIONE\_\allowbreak{}ESPLORAZIONE=5, la stessa della versione a 2 messaggi, 13500 episodi): risultato parziale ma consistente -\/- tutti e tre i messaggi mostrano lo STESSO pattern (rispondono su A turno 3, B turno 5, A turno 7, ma falliscono su B turno 1), 9 scambi su 12 riusciti. La mappa dei pesi pero\textquotesingle{} rivela il motivo: un\textquotesingle unica associazione di secondo ordine domina a spese delle tre righe primarie, che restano deboli -\/- con 3 messaggi (che generano anche le proprie associazioni derivate) la richiesta di "slot" distinti sull\textquotesingle anello supera quello che una repulsione tarata per 2 messaggi riesce a separare.

Secondo tentativo, per rinforzare la separazione: REPULSIONE\_\allowbreak{}ESPLORAZIONE portata a 20 (4x) e budget aumentato ulteriormente (18000 episodi). Risultato: la mappa dei pesi finale mostra ORA tre righe distinte correttamente posizionate vicino a x=0.1, 0.4, 0.7 -\/- la separazione e\textquotesingle{} migliorata, esattamente come previsto. Ma il tasso di successo e\textquotesingle{} PEGGIORATO (6 scambi su 12 invece di 9), e il picco di W\_\allowbreak{}AB e\textquotesingle{} crollato a 0.0067 (contro 0.0411 del primo tentativo) -\/- il canale B non si accende quasi mai nemmeno in valutazione pulita. Una repulsione troppo forte non discrimina tra "zona occupata da un ALTRO messaggio" (da evitare) e "piccola variazione naturale intorno alla propria posizione" (da non penalizzare): finisce per respingere anche il rinforzo legittimo, non solo le collisioni, impedendo alle righe di consolidarsi abbastanza da superare la soglia di accensione autonoma.

Conclusione: separazione delle associazioni e forza del consolidamento sono due obiettivi in TENSIONE, non allineati come ci si aspetterebbe -\/- rinforzare la repulsione ottiene l\textquotesingle uno a spese dell\textquotesingle altro. Il valore giusto e\textquotesingle{} probabilmente intermedio tra 5 e 20, non ancora trovato in questo lavoro: un risultato onesto ma incompleto, lasciato come domanda aperta piuttosto che forzato in una conclusione piu\textquotesingle{} comoda di quella osservata.

\includegraphics[width=5.8in,height=4.35in]{media/media/image29.png}

\emph{Fig. 28 --- Scalabilita\textquotesingle{} a 3 messaggi con repulsione rinforzata: righe meglio separate, ma consolidamento piu\textquotesingle{} debole (6/12 invece di 9/12).}

\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}multi\_\allowbreak{}repulsione\_\allowbreak{}3msg.py}}

\lstinputlisting[language=Python]{scripts/dialogo_multi_repulsione_3msg.py}
\subsubsection{5.5.9 Correzione risolutiva: repulsione solo per la scoperta, esplorazione locale per il consolidamento}\label{correzione-risolutiva-repulsione-solo-per-la-scoperta-esplorazione-locale-per-il-consolidamento}

Tre tentativi precedenti (5.5.6 con repulsione debole, 5.5.8 con repulsione forte fissa e con annealing) avevano mostrato una tensione irrisolta: una repulsione abbastanza forte da separare bene le tre righe primarie lasciava il consolidamento debole (picco W\_\allowbreak{}AB \textasciitilde0.006-0.007), mentre una repulsione debole permetteva un consolidamento forte ma con le righe che collidevano (una sola associazione derivata dominante). Diagnosi riveduta: il problema non era la FORZA della repulsione ma il fatto che restasse attiva per TUTTO il training. Anche dopo che un messaggio ha gia\textquotesingle{} trovato una zona, ogni nuovo tentativo esplorativo veniva ripescato dalla distribuzione repulsiva GLOBALE -\/- che si sposta leggermente ogni volta che ALTRI messaggi si allenano -\/- impedendo al rinforzo di concentrarsi in un punto stabile anche a separazione gia\textquotesingle{} ottenuta.

Correzione: due fasi per messaggio, non una sola strategia fissa per tutto il training. Finche\textquotesingle{} un messaggio non ha ancora trovato una prima risposta valida, esplorazione repulsiva (come prima, serve a separare le scoperte iniziali). Una volta trovata una prima risposta, il messaggio passa a un\textquotesingle esplorazione LOCALE -\/- un piccolo jitter attorno alla posizione gia\textquotesingle{} scoperta -\/- invece di continuare a ripescare dalla distribuzione globale: il consolidamento non dipende piu\textquotesingle{} da cosa succede altrove nella matrice mentre gli altri messaggi si allenano.

Risultato (stessa configurazione delle prove precedenti: 13500 episodi, round-robin, 3 messaggi): 10 scambi su 12 riusciti -\/- il migliore finora, contro i 9/12 del primo tentativo con repulsione debole. Due messaggi su tre (x=0.10 e x=0.40) arrivano al 100\% su TUTTI e quattro gli scambi, con un dettaglio notevole: i rispettivi offset appresi (0.300 e 0.295) coincidono quasi esattamente col bias innato (0.3) -\/- una convergenza molto piu\textquotesingle{} pulita di qualunque run precedente, dove l\textquotesingle offset finale tendeva a discostarsi parecchio dal bias di partenza. Il terzo messaggio (x=0.70) arriva a 2/4 (B turno 1 e A turno 3 riusciti; B turno 5 e A turno 7 no). Le curve di allenamento mostrano anche una dinamica qualitativamente diversa e piu\textquotesingle{} sana rispetto a tutti i tentativi precedenti: x=0.10 e x=0.40 raggiungono il 100\% (episodio \textasciitilde1500 e \textasciitilde2000) e ci restano stabilmente fino alla fine, SENZA la continua oscillazione osservata in ogni tentativo precedente. La mappa dei pesi finale mostra tre macchie nitide, compatte e ben separate -\/- non piu\textquotesingle{} bande sfocate o un unico picco dominante.

Il residuo (x=0.70 sulle associazioni di ordine piu\textquotesingle{} alto) e\textquotesingle{} verosimilmente solo una questione di fortuna nell\textquotesingle ordine di scoperta (nel round-robin x=0.70 e\textquotesingle{} sempre l\textquotesingle ultimo dei tre in ogni ciclo) piuttosto che un limite strutturale nuovo -\/- coerente con quanto gia\textquotesingle{} osservato altrove nel documento (le associazioni di ordine piu\textquotesingle{} alto richiedono sistematicamente piu\textquotesingle{} episodi delle prime, sezione 5.5.3). Separare la fase di scoperta da quella di consolidamento risolve quindi la tensione centrale diagnosticata in 5.5.8, senza il compromesso osservato nei tentativi con un\textquotesingle unica strategia fissa per tutto il training.

\includegraphics[width=5.8in,height=4.35in]{media/media/image30.png}

\emph{Fig. 29 --- Scoperta repulsiva + consolidamento locale: 10/12 scambi, due messaggi su tre perfetti, mappa dei pesi con tre macchie nitide e separate.}

\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}multi\_\allowbreak{}scoperta\_\allowbreak{}locale.py}}

\lstinputlisting[language=Python]{scripts/dialogo_multi_scoperta_locale.py}
\subsubsection{5.5.10 Verifica Monte Carlo: il risultato e\textquotesingle{} robusto, ma l\textquotesingle ipotesi sul residuo era sbagliata}\label{verifica-monte-carlo-il-risultato-e-robusto-ma-lipotesi-sul-residuo-era-sbagliata}

La sezione 5.5.9 aveva ottenuto 10 scambi su 12 con un solo seed casuale, e aveva attribuito il residuo (il terzo messaggio, ultimo del round-robin, che non raggiungeva le associazioni di ordine piu\textquotesingle{} alto) a sfortuna nell\textquotesingle ordine di scoperta. Un singolo run non basta a confermarlo -\/- lo stesso principio statistico gia\textquotesingle{} applicato nel progetto (PSTH su 25 prove indipendenti, sezione 2.11) richiede di ripetere l\textquotesingle esperimento con SEED diversi prima di trarre conclusioni sul perche\textquotesingle{} un risultato va in un certo modo. Otto repliche indipendenti (stessa configurazione esatta, nessuna modifica all\textquotesingle algoritmo, solo il seed di rng\_\allowbreak{}train) sono state eseguite in parallelo.

Risultato aggregato: 88 successi su 96 controlli totali (8 seed x 3 messaggi x 4 scambi) = 91.7\% -\/- piu\textquotesingle{} alto della singola run originale (83\%), confermando che il meccanismo di scoperta-poi-consolidamento-locale e\textquotesingle{} robusto, non un colpo di fortuna isolato. Il turno 1 (B risponde) e il turno 3 (A risponde) sono risultati PERFETTAMENTE affidabili: 100\% per tutti e tre i messaggi, in tutti e 8 i seed senza eccezioni.
...
\bottomrule\noalign{}
\endlastfoot
x=0.10 (primo) & 100\% & 100\% & 100\% & 100\% \\
x=0.40 (secondo) & 100\% & 100\% & 37.5\% & 100\% \\
x=0.70 (ultimo) & 100\% & 100\% & 75\% & 87.5\% \\
\end{longtable}
}

L\textquotesingle ipotesi "sfortuna nell\textquotesingle ordine di scoperta, sempre l\textquotesingle ultimo a soffrirne" non regge di fronte ai dati aggregati. Il primo messaggio del round-robin (x=0.10) e\textquotesingle{} risultato perfetto su tutti e quattro gli scambi in TUTTI gli 8 seed, senza una sola eccezione -\/- il piu\textquotesingle{} affidabile in assoluto. Ma il piu\textquotesingle{} fragile non e\textquotesingle{} l\textquotesingle ultimo (x=0.70, 75-87.5\% sulle associazioni di ordine piu\textquotesingle{} alto) -\/- e\textquotesingle{} il SECONDO (x=0.40), che fallisce sul turno 5 in 5 seed su 8 (37.5\% di affidabilita\textquotesingle). Un pattern non-monotono rispetto alla posizione nel round-robin, diverso da quanto ipotizzato: il primo messaggio ha un vantaggio strutturale reale (probabilmente accede sempre per primo a una matrice ancora completamente libera, in ogni ciclo di training), ma tra il secondo e il terzo posto la relazione non e\textquotesingle{} semplicemente "peggio quanto piu\textquotesingle{} tardi". La causa esatta di questa asimmetria resta una domanda aperta, corretta rispetto alla spiegazione troppo semplice proposta in 5.5.9 -\/- un altro caso, nello stesso spirito dichiarato in apertura di questo documento, in cui verificare rigorosamente ha smentito una spiegazione plausibile ma non controllata.

\includegraphics[width=5.8in,height=3.625in]{media/media/image31.png}

\emph{Fig. 30 --- Tasso di successo per messaggio e scambio, aggregato su 8 seed indipendenti: B(t1) e A(t3) perfetti al 100\%, B(t5) e A(t7) variabili e non-monotoni rispetto alla posizione nel round-robin.}

\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}multi\_\allowbreak{}scoperta\_\allowbreak{}locale\_\allowbreak{}montecarlo.py}}

\lstinputlisting[language=Python]{scripts/dialogo_multi_scoperta_locale_montecarlo.py}
\subsubsection{5.5.11 Disambiguazione posizione/valore: due effetti distinti, non uno}\label{disambiguazione-posizionevalore-due-effetti-distinti-non-uno}

La sezione 5.5.10 aveva lasciato una domanda aperta: la fragilita\textquotesingle{} osservata sulle associazioni di ordine piu\textquotesingle{} alto segue la POSIZIONE nel round-robin o il VALORE del messaggio? Il test diretto: ripetere le stesse 8 repliche con l\textquotesingle ordine del round-robin RUOTATO -\/- {[}0.40, 0.70, 0.10{]} invece di {[}0.10, 0.40, 0.70{]}, stesso codice, stessi seed (101-108) -\/- e confrontare quale dei due (posizione o valore) resta costante.
...
\begin{minipage}[b]{\linewidth}\raggedright
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Ordine originale {[}0.10, 0.40, 0.70{]}
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
Ordine permutato {[}0.40, 0.70, 0.10{]}
\end{minipage} \\
\midrule\noalign{}
\endhead
\bottomrule\noalign{}
\endlastfoot
Posizione 1 & x=0.10: B5=100\% A7=100\% & x=0.40: B5=50\% A7=100\% \\
Posizione 2 & x=0.40: B5=37.5\% A7=100\% & x=0.70: B5=37.5\% A7=87.5\% \\
Posizione 3 & x=0.70: B5=75\% A7=87.5\% & x=0.10: B5=50\% A7=100\% \\
\end{longtable}
}

Il risultato disambigua in modo netto DUE effetti distinti, che nella prima osservazione (5.5.9-5.5.10) apparivano confusi insieme:

Il turno 5 (B risponde di nuovo) segue la POSIZIONE: chi occupa la posizione 2 del round-robin ottiene ESATTAMENTE 37.5\% di affidabilita\textquotesingle{} in entrambi gli ordini -\/- prima era il messaggio x=0.40 a trovarsi li\textquotesingle, ora e\textquotesingle{} x=0.70, ma il tasso di successo e\textquotesingle{} identico alla prima cifra decimale. E\textquotesingle{} un effetto puro dell\textquotesingle ordine temporale di allenamento (probabilmente legato a come si sovrappongono le fasi di scoperta dei tre messaggi nel tempo), non del valore specifico.

Il turno 7 (A risponde di nuovo) segue invece il VALORE: il messaggio x=0.70 resta a 87.5\% sia quando occupa la posizione 3 (ordine originale) sia quando occupa la posizione 2 (ordine permutato) -\/- mentre x=0.10 e x=0.40 restano sempre al 100\% indipendentemente da dove vengono collocati nel round-robin. E\textquotesingle{} un effetto legato alla posizione SULL\textquotesingle ANELLO di x=0.70 specificamente (forse un\textquotesingle interazione con la geometria discretizzata, nello stesso spirito del bias di posizione gia\textquotesingle{} documentato per il ring attractor in altre sezioni), non all\textquotesingle ordine di allenamento.

Una disambiguazione pulita, ottenuta con un controllo diretto invece che con altre ipotesi non testate: due fenomeni indipendenti si sovrapponevano nell\textquotesingle osservazione originale, ciascuno legato a una causa diversa (ordine temporale per il turno 5, valore/posizione sull\textquotesingle anello per il turno 7). Nessuno dei due e\textquotesingle{} stato risolto qui -\/- restano entrambi documentati come proprieta\textquotesingle{} note del sistema, non piu\textquotesingle{} come un\textquotesingle unica domanda aperta confusa.

\includegraphics[width=5.8in,height=2.63636in]{media/media/image32.png}

\emph{Fig. 31 --- Confronto diretto tra i due ordini di round-robin: il turno 5 segue la posizione (37.5\% costante in posizione 2), il turno 7 segue il valore (87.5\% costante per x=0.70).}

\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}multi\_\allowbreak{}scoperta\_\allowbreak{}locale\_\allowbreak{}permutazione.py}}

\lstinputlisting[language=Python]{scripts/dialogo_multi_scoperta_locale_permutazione.py}
\subsubsection{5.5.12 Tentativo di correzione dell\textquotesingle effetto posizionale: piu\textquotesingle{} equo, ma non migliore}\label{tentativo-di-correzione-delleffetto-posizionale-piu-equo-ma-non-migliore}

La sezione 5.5.11 aveva isolato un effetto posizionale puro sul turno 5: chi occupa la posizione 2 del round-robin FISSO ({[}0.10, 0.40, 0.70{]} ripetuto identico ogni ciclo) ottiene 37.5\% di affidabilita\textquotesingle, indipendentemente da quale messaggio la occupi. Correzione tentata: RIMESCOLARE l\textquotesingle ordine del ciclo a ogni giro invece di ripeterlo fisso -\/- nessun messaggio dovrebbe restare bloccato nella stessa posizione per tutto il training. Stesso protocollo di verifica: 8 seed indipendenti (101-108).
...
Risultato onesto ma non quello sperato: il totale aggregato SCENDE a 87.5\% (84/96), contro il 91.7\% (88/96) dell\textquotesingle ordine fisso originale (sezione 5.5.10). La correzione raggiunge pero\textquotesingle{} il suo obiettivo ristretto -\/- nessun messaggio resta piu\textquotesingle{} sistematicamente perfetto o sistematicamente svantaggiato come prima (x=0.10 era perfetto al 100\% sul turno 5 nell\textquotesingle ordine fisso, ora scende al 75\%; x=0.40 sale dal 37.5\% al 50\%) -\/- ma mescolare introduce piu\textquotesingle{} variabilita\textquotesingle{} di quanta ne rimuova: ogni messaggio, in momenti diversi del training, capita comunque a fianco di un ordine sfavorevole, e la somma di questi episodi sfavorevoli per TUTTI e tre pesa di piu\textquotesingle{} del vantaggio strutturale che prima si concentrava tutto su UN messaggio soltanto. Un trade-off genuino tra equita\textquotesingle{} (nessuno sistematicamente svantaggiato) e prestazione aggregata (l\textquotesingle ordine fisso, pur ingiusto verso un messaggio specifico, restava complessivamente piu\textquotesingle{} affidabile) -\/- documentato onestamente come risultato negativo rispetto all\textquotesingle obiettivo di migliorare il totale, nello stesso spirito dichiarato in apertura di questo documento.

\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}multi\_\allowbreak{}scoperta\_\allowbreak{}locale\_\allowbreak{}mischiato.py}}

\lstinputlisting[language=Python]{scripts/dialogo_multi_scoperta_locale_mischiato.py}
\subsubsection{5.5.13 Seconda correzione: estendere la scoperta+consolidamento locale al canale di ritorno}\label{seconda-correzione-estendere-la-scopertaconsolidamento-locale-al-canale-di-ritorno}

Il tentativo sull\textquotesingle effetto posizionale (5.5.12) non era bastato -\/- rimaneva ancora da affrontare l\textquotesingle effetto legato al VALORE (x=0.70 fragile all\textquotesingle87.5\% sul turno 7, ovunque collocato nell\textquotesingle ordine, sezione 5.5.11). Osservazione strutturale: il meccanismo a due fasi (repulsione per la scoperta iniziale, poi esplorazione locale per il consolidamento) introdotto in dialogo\_\allowbreak{}multi\_\allowbreak{}scoperta\_\allowbreak{}locale.py era stato applicato SOLO al canale A-\textgreater B (turni 1 e 5) -\/- il canale di ritorno B-\textgreater A (turni 3 e 7) era rimasto con la sola ricerca repulsiva per tutta la durata del training, la stessa condizione che aveva causato instabilita\textquotesingle{} sul canale A-\textgreater B prima della correzione. Estensione naturale: applicare lo stesso trattamento a due fasi anche al canale di ritorno (scoperta\_\allowbreak{}A, analogo a scoperta\_\allowbreak{}B).
...
Risultato (8 seed indipendenti, stesso protocollo delle sezioni precedenti): 92 successi su 96 controlli = 95.8\%, il migliore finora -\/- contro il 91.7\% dell\textquotesingle ordine fisso originale e l\textquotesingle87.5\% del tentativo con ordine mescolato. Il turno 7 (l\textquotesingle effetto legato al valore x=0.70) e\textquotesingle{} ORA RISOLTO al 100\%, senza eccezioni in nessuno degli 8 seed -\/- il primo dei due effetti identificati in 5.5.11 completamente eliminato, non solo attenuato. Il turno 5 (l\textquotesingle effetto posizionale) migliora anche lui, da una media del 70.8\% all\textquotesingle83.3\%, e diventa molto piu\textquotesingle{} uniforme tra i messaggi (75-87.5\% invece del 37.5-100\% originale, dove un messaggio era perfetto e un altro molto fragile) -\/- non risolto del tutto, ma sensibilmente attenuato E reso piu\textquotesingle{} equo, senza il compromesso negativo osservato nel tentativo precedente (5.5.12, dove rendere il sistema piu\textquotesingle{} equo costava un totale piu\textquotesingle{} basso).

Il fatto che estendere il canale di ritorno abbia migliorato ANCHE il turno 5 (che dipende dal canale A-\textgreater B, non da quello corretto qui) e\textquotesingle{} un indizio interessante: i due canali, pur allenati con dinamiche separate, non sono del tutto indipendenti nella pratica -\/- un canale di ritorno piu\textquotesingle{} stabile sembra ridurre indirettamente il rumore accumulato anche sul canale diretto. Una connessione non esplorata ulteriormente in questo lavoro, lasciata come osservazione per sviluppi futuri.

\includegraphics[width=5.8in,height=3.22222in]{media/media/image33.png}

\emph{Fig. 32 --- Confronto diretto tra ordine fisso originale (91.7\%) e correzione bidirezionale (95.8\%): turno 7 risolto al 100\%, turno 5 migliorato e piu\textquotesingle{} uniforme.}

\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}multi\_\allowbreak{}scoperta\_\allowbreak{}bidirezionale.py}}
...
\textit{\textbf{Codice sorgente: ring\_\allowbreak{}attractor\_\allowbreak{}spiking.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}turni.py}}

\lstinputlisting[language=Python]{scripts/dialogo_spiking_bidirezionale_turni.py}
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}gate.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}multi.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}multi\_\allowbreak{}montecarlo.py}}
...
\textit{\textbf{Codice sorgente: diagnosi\_\allowbreak{}sottosoglia\_\allowbreak{}ba.py}}

\lstinputlisting[language=Python]{scripts/diagnosi_sottosoglia_ba.py}
\textit{\textbf{Codice sorgente: diagnosi\_\allowbreak{}ablazione\_\allowbreak{}ba.py}}

\lstinputlisting[language=Python]{scripts/diagnosi_ablazione_ba.py}
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}silenzia.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}interruttore.py}}

\lstinputlisting[language=Python]{scripts/dialogo_spiking_bidirezionale_interruttore.py}
\textit{\textbf{Codice sorgente: diagnosi\_\allowbreak{}unidirezionale\_\allowbreak{}pesi\_\allowbreak{}forti.py}}

\lstinputlisting[language=Python]{scripts/diagnosi_unidirezionale_pesi_forti.py}
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}interruttore\_\allowbreak{}mc.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}omeostasi.py}}

\lstinputlisting[language=Python]{scripts/dialogo_spiking_bidirezionale_omeostasi.py}
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}omeostasi\_\allowbreak{}mc.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}omeostasi\_\allowbreak{}v2.py}}

\lstinputlisting[language=Python]{scripts/dialogo_spiking_bidirezionale_omeostasi_v2.py}
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}omeostasi\_\allowbreak{}v2\_\allowbreak{}mc.py}}
...
\textit{\textbf{Codice sorgente: diagnosi\_\allowbreak{}seed301\_\allowbreak{}anello\_\allowbreak{}isolato.py}}

\lstinputlisting[language=Python]{scripts/diagnosi_seed301_anello_isolato.py}
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}omeostasi\_\allowbreak{}v3.py}}

\lstinputlisting[language=Python]{scripts/dialogo_spiking_bidirezionale_omeostasi_v3.py}
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}omeostasi\_\allowbreak{}v3\_\allowbreak{}mc.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}omeostasi\_\allowbreak{}v4.py}}

\lstinputlisting[language=Python]{scripts/dialogo_spiking_bidirezionale_omeostasi_v4.py}
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}omeostasi\_\allowbreak{}v4\_\allowbreak{}mc.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}multi.py}}

\lstinputlisting[language=Python]{scripts/dialogo_spiking_bidirezionale_multi.py}
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}multi\_\allowbreak{}mc.py}}
...
Quattordicesimo tentativo: irrigidimento del carico della sezione 5.18, da due a tre messaggi -\/- lo stesso passo che nel modello a rate aveva fatto emergere per la prima volta un\textquotesingle interferenza seria (dialogo\_\allowbreak{}multi\_\allowbreak{}repulsione\_\allowbreak{}3msg.py, sezione 5.5.8). Stesse tre posizioni di quel test: x=0.10, 0.40, 0.70. Schema a turni esteso a 6 combinazioni cicliche (messaggio, direzione), 20 ripetizioni ciascuna (120 turni, 120 secondi di simulazione).
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}multi3.py}}

\lstinputlisting[language=Python]{scripts/dialogo_spiking_bidirezionale_multi3.py}
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}multi3\_\allowbreak{}mc.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}relay\_\allowbreak{}3anelli.py}}

\lstinputlisting[language=Python]{scripts/dialogo_spiking_relay_3anelli.py}
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}relay\_\allowbreak{}3anelli\_\allowbreak{}mc.py}}
...
Codice sorgente completo, README con metodologia/limiti/sviluppi futuri e dati grezzi: \textasciitilde/Desktop/geant4\_\allowbreak{}cnao\_\allowbreak{}ring/ (progetto Geant4/C++ separato, non incluso nel repository GitHub Python di neuroni-progetto).
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}knockout.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}knockout\_\allowbreak{}mc.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}knockout\_\allowbreak{}controllo\_\allowbreak{}mc.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}knockout\_\allowbreak{}v2\_\allowbreak{}mc.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}knockout\_\allowbreak{}picco\_\allowbreak{}mc.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}knockout\_\allowbreak{}proton\_\allowbreak{}mc.py}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}recupero.py}}
...
Domanda di confine naturale dopo la 5.29: il recupero funzionale visto li\textquotesingle{} usava un knockout parziale (10 neuroni a dose piu\textquotesingle{} alta da uno scenario CNAO reale, di cui solo 4 dentro la zona di codifica di x=0.65). Se il danno viene spinto al limite -\/- spenti TUTTI E 11 i neuroni della zona di codifica (l\textquotesingle intero gruppo\_\allowbreak{}stim), non piu\textquotesingle{} uno scenario di dose fisico ma un test di confine puro -\/- il meccanismo di recupero regge ancora, o esiste un punto oltre il quale la plasticita\textquotesingle{} residua non ha piu\textquotesingle{} nessun percorso alternativo da sfruttare?
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}lesione\_\allowbreak{}completa\_\allowbreak{}mc.py (verifica 4 seed)}}
...
Implementato in Geant4 (SteppingAction/EventAction/RunAction): il tracking del LET dose-mediato per neurone, Sum(dE\_\allowbreak{}i\^{}2/dx\_\allowbreak{}i)/Sum(dE\_\allowbreak{}i) (la stessa quantita\textquotesingle{} usata nel modello di Kanai), accumulato passo per passo insieme alla dose fisica gia\textquotesingle{} esistente. In analisi, applicata una conversione RBE(LET) semplificata -\/- lineare, ANCORATA al punto di riferimento NIRS (RBE=3.0 a 80 keV/um), saturata a RBE\_\allowbreak{}max=3.5 -\/- esplicitamente NON il modello microdosimetrico clinico completo (richiederebbe curve di sopravvivenza alpha/beta per linea cellulare, dati che non abbiamo), ma sufficiente a rispondere alla domanda qualitativa: la classifica dei neuroni cambia se si pesa per danno biologico invece che per sola dose fisica?
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}knockout\_\allowbreak{}media4seed\_\allowbreak{}mc.py}}
...
\textit{\textbf{Codice sorgente: geant4\_\allowbreak{}cnao\_\allowbreak{}ring/analysis/mkm\_\allowbreak{}rbe.py}}
...
Metodologicamente: la diagnostica per-step resta nel codice (`SteppingAction.cc`), attivabile per qualunque neurone tramite la variabile d\textquotesingle ambiente CNAO\_\allowbreak{}DEBUG\_\allowbreak{}NEURON=\textless indice\textgreater{} -\/- lo stesso principio di verificare empiricamente un risultato sorprendente invece di limitarsi a ipotizzarne la causa, applicato qui al lato Geant4 con lo stesso rigore gia\textquotesingle{} usato sistematicamente lato Brian2 in tutto il progetto.

\textit{\textbf{Codice sorgente: geant4\_\allowbreak{}cnao\_\allowbreak{}ring/src/SteppingAction.cc (diagnostica CNAO\_\allowbreak{}DEBUG\_\allowbreak{}NEURON aggiunta in questa sezione)}}
...
\emph{\textbf{Dati generati con cnaoRingImpact (seed 501/502, 5.000.000 eventi ciascuno) in build/results\_\allowbreak{}highstat/}}
...
\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}knockout\_\allowbreak{}top5\_\allowbreak{}mc.py e dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}knockout\_\allowbreak{}top20\_\allowbreak{}mc.py}}
...
Ultimo sviluppo futuro rimasto nel README di geant4\_\allowbreak{}cnao\_\allowbreak{}ring: collegare questo progetto (fisica macroscopica, QBBC) al progetto gemello geant4dna\_\allowbreak{}icsd (nanodosimetria, Geant4-DNA, verifica del paper Villagrasa/Baiocco 2026), ri-simulando in dettaglio la traccia degli elettroni secondari prodotti nei neuroni colpiti -\/- lo stesso approccio "track re-simulation" multi-scala usato da TOPAS-nBio. Prima di costruire la pipeline completa, una domanda va verificata: gli elettroni secondari prodotti da un fascio clinico CNAO dentro un neurone hanno davvero energie compatibili con Geant4-DNA (valido solo fino a \textasciitilde1 MeV)?
...
Corretto aggiungendo una G4Region dedicata ai volumi "Neuron", con un taglio di produzione di 100nm (sotto la scala del neurone). Ripetuta la stessa configurazione (carbonio-12 al picco): 18.021 elettroni secondari espliciti su 500.000 eventi. Spettro: minimo 0.99 keV, massimo 96.2 keV, mediana 1.86 keV, media 3.57 keV -\/- il 100\% sotto 1 MeV, dentro il dominio di validita\textquotesingle{} di Geant4-DNA. Coincidenza notevole: la mediana e la media cadono proprio nell\textquotesingle intervallo di energie (1, 5, 10 keV) testate esplicitamente nel paper Villagrasa/Baiocco che geant4dna\_\allowbreak{}icsd riproduce -\/- il collegamento non richiederebbe estrapolare oltre il dominio gia\textquotesingle{} validato in quel progetto gemello.
...
\emph{\textbf{Verificato con nanoICSD (seed espliciti 201-204), geant4dna\_\allowbreak{}icsd/build/results\_\allowbreak{}mc\_\allowbreak{}seed\_\allowbreak{}check/}}
...
\emph{\textbf{Eseguito con moleculardna (Geant4-DNA collaboration), geant4dna\_\allowbreak{}ssbdsb/build/, energie dallo spettro reale della sezione 5.39}}
...
\emph{\textbf{Eseguito con moleculardna (geant4dna\_\allowbreak{}ssbdsb/build/results\_\allowbreak{}spectrum\_\allowbreak{}weighted/), 6 punti pesati sullo spettro reale della sezione 5.39}}
...
Le sezioni 5.39-5.43 hanno costruito e verificato l\textquotesingle intera catena nanodosimetrica (spettro di elettroni secondari -\textgreater{} ICSD -\textgreater{} danno reale al DNA), ma solo a livello AGGREGATO sull\textquotesingle intero anello -\/- mai collegata neurone per neurone al criterio di knockout usato nelle sezioni 5.22-5.38 (sempre basato sulla dose fisica). Questa sezione chiude quel collegamento: esteso il tracking di geant4\_\allowbreak{}cnao\_\allowbreak{}ring per registrare, per ciascuno dei 100 neuroni, quanti elettroni secondari nasce al suo interno e una stima del danno DSB atteso (assegnando a ogni elettrone il tasso DSB/primario del punto rappresentativo piu\textquotesingle{} vicino tra i 6 della sezione 5.43 -\/- una stima che riusa i tassi gia\textquotesingle{} validati, non una nuova simulazione di danno per ciascun neurone).
...
\textit{\textbf{Codice sorgente: geant4\_\allowbreak{}cnao\_\allowbreak{}ring (EventAction.cc/RunAction.cc, tracking per-neurone aggiunto in questa sezione)}}

\lstinputlisting[language=C++]{geant4_src/EventAction.cc}
\lstinputlisting[language=C++]{geant4_src/RunAction.cc}
\subsection{5.45 Il loop C-\textgreater A non si chiude: la staffetta a tre anelli non si autosostiene}\label{il-loop-c-a-non-si-chiude-la-staffetta-a-tre-anelli-non-si-autosostiene}

Torna al filone di neuroscienza pura, chiudendo una domanda aperta dalla sezione 5.20: la staffetta direzionale A-\textgreater B-\textgreater C-\textgreater A (tre popolazioni, solo collegamenti unidirezionali con STDP, nessuna delle instabilita\textquotesingle{} di retroazione mutua incontrate con la vera bidirezionalita\textquotesingle{} puo\textquotesingle{} presentarsi per costruzione) propaga correttamente l\textquotesingle informazione di posizione da A fino a C dopo due salti -\/- ma il collegamento di chiusura C-\textgreater A trasmette in modo CAUSALE, capace di sostenere un\textquotesingle eco del messaggio anche senza piu\textquotesingle{} stimolo esterno su A?

Protocollo: 80 ripetizioni normali di allenamento (stimolo esterno solo su A, come nella 5.20), seguite da 10 cicli SONDA consecutivi senza alcuno stimolo esterno su nessun anello. Se C-\textgreater A e\textquotesingle{} causalmente efficace, A dovrebbe mostrare attivita\textquotesingle{} tardiva stabile in OGNUNO dei 10 cicli sonda (un\textquotesingle eco autosostenuta dal loop); se invece l\textquotesingle apparente chiusura fosse un artefatto o pura persistenza residua di A stessa, l\textquotesingle attivita\textquotesingle{} dovrebbe esaurirsi entro i primi 1-2 cicli e non ricomparire.

Risultato, verificato su 5 run indipendenti (1 run singolo + 4 seed Monte Carlo): mai piu\textquotesingle{} di 3 sonde su 10 mostrano una qualunque attivita\textquotesingle{} in A, e quando ne mostrano e\textquotesingle{} sempre 1-2 spike isolati -\/- rumore di fondo della rete (400 Hz di stimolo Poisson di base su ogni neurone), non un segnale riconoscibile. Nessuno dei 5 run mostra la firma attesa di un loop causale (attivita\textquotesingle{} stabile e localizzata in OGNI ciclo sonda).

seed singolo (run originale): 1/10 sonde con spike (solo la prima, 2 spike); seed 1: 3/10 (1 spike ciascuna); seed 2: 0/10; seed 3: 2/10 (1 spike ciascuna); seed 4: 0/10.

Conclusione: il collegamento C-\textgreater A non chiude un loop causale autosostenuto in questa architettura -\/- coerente con, ed estensione di, la conclusione gia\textquotesingle{} raggiunta nella sezione 5.9 per il caso piu\textquotesingle{} semplice a due anelli (il canale di ritorno B-\textgreater A non trasmette causalmente in questo sistema). Anche con un terzo anello intermedio e un percorso di ritorno piu\textquotesingle{} lungo, lo stesso limite strutturale si ripresenta: la propagazione unidirezionale in avanti (A-\textgreater B-\textgreater C) funziona in modo affidabile, ma nessun percorso di ritorno testato finora in questo progetto -\/- ne\textquotesingle{} diretto (B-\textgreater A, 5.9) ne\textquotesingle{} attraverso un anello intermedio (C-\textgreater A, qui) -\/- si dimostra causalmente efficace senza la stabilizzazione esplicita (interruttore di trasmissione, normalizzazione omeostatica) sviluppata per la vera bidirezionalita\textquotesingle{} nelle sezioni 5.13-5.17.

\textit{\textbf{Codice sorgente: dialogo\_\allowbreak{}spiking\_\allowbreak{}relay\_\allowbreak{}chiusura\_\allowbreak{}turni\_\allowbreak{}mc.py}}

\lstinputlisting[language=Python]{scripts/dialogo_spiking_relay_chiusura_turni_mc.py}
\subsection{5.46 La coda di frammentazione del carbonio-12: protoni e carbonio a confronto oltre il picco}\label{la-coda-di-frammentazione-del-carbonio-12-protoni-e-carbonio-a-confronto-oltre-il-picco}

Un aspetto della fisica del carbonio-12 mai affrontato nelle sezioni 5.21-5.45, ma centrale nella letteratura clinica e nella storia stessa di Chiba/NIRS-HIMAC (il primo centro al mondo a trattare pazienti con ioni di carbonio, dal 1994, sezione 5.31): a differenza di un protone, uno ione di carbonio-12 che attraversa il tessuto puo\textquotesingle{} subire reazioni nucleari inelastiche con i nuclei bersaglio, frammentandosi in particelle piu\textquotesingle{} leggere (protoni, particelle alfa, deutoni, tritoni, o isotopi piu\textquotesingle{} pesanti come litio, berillio, boro). Questi frammenti, avendo un range piu\textquotesingle{} lungo del proprio ione genitore alla stessa velocita\textquotesingle, continuano a depositare dose ANCHE OLTRE il picco di Bragg del fascio primario -\/- la cosiddetta "coda di frammentazione", uno dei compromessi fisici piu\textquotesingle{} discussi nel confronto clinico tra ioni di carbonio e protoni, e uno dei motivi per cui l\textquotesingle RBE piu\textquotesingle{} alto del carbonio al bersaglio (sezioni 5.31-5.35) non e\textquotesingle{} un vantaggio a costo zero.

Metodo: usato braggProfile (fascio largo 5x5cm, 350 fette da 1mm, gia\textquotesingle{} validato nella sezione 5.25) per confrontare il profilo di dose in profondita\textquotesingle{} OLTRE il picco per le stesse due configurazioni gia\textquotesingle{} usate in tutto il filone CNAO -\/- carbonio-12 a 120 MeV/u (picco a 34.5mm) e protone a 70 MeV (picco a 39.5mm) -\/- la scelta di energie della sezione 5.28, per restare confrontabili con il resto del lavoro. La geometria a fascio largo, gia\textquotesingle{} usata per trovare i picchi di Bragg nella sezione 5.25, da\textquotesingle{} una statistica enormemente migliore di quella per-neurone di cnaoRingImpact per un effetto raro come questo (si veda oltre).

Risultato: il carbonio-12 mostra una coda sostanziale e a decadimento lento -\/- dal 2.0\% del picco a soli 5mm oltre il proprio massimo, scende gradualmente fino allo 0.05\% a 100mm di distanza, senza mai crollare a zero. Il protone mostra invece un crollo quasi immediato: gia\textquotesingle{} a 5mm oltre il proprio picco la dose residua e\textquotesingle{} scesa a circa 0.007\% del massimo -\/- circa 300 volte meno del carbonio alla stessa distanza relativa -\/- e resta a quel livello rumoroso (statistica limitata, coerente col fatto che l\textquotesingle evento e\textquotesingle{} raro) per tutta la profondita\textquotesingle{} testata, senza la coda liscia e persistente del carbonio. Il rapporto tra le due code, misurato a margini diversi oltre il rispettivo picco (5, 10, 15, 20, 30, 50mm), oscilla tra 120x e 540x a seconda del punto esatto (la statistica del protone e\textquotesingle{} rumorosa bin per bin, coerente con un evento raro), ma la conclusione qualitativa e\textquotesingle{} netta e stabile su tutto l\textquotesingle intervallo: la coda di frammentazione del carbonio e\textquotesingle{} un effetto fisico reale, di ordini di grandezza piu\textquotesingle{} grande di qualunque cosa osservabile per un fascio di protoni alla stessa energia relativa.

\includegraphics[width=6in,height=4in]{media/media/image54.png}

\emph{Fig. 53 --- Coda di frammentazione oltre il picco di Bragg: dose relativa al picco (scala log) in funzione della profondita\textquotesingle{} oltre il proprio picco, carbonio-12 120 MeV/u contro protone 70 MeV. Il carbonio mostra una coda liscia e persistente; il protone crolla quasi subito a un livello rumoroso \textasciitilde300x piu\textquotesingle{} basso.}

Conferma alla scala del singolo neurone: esteso il tracking di geant4\_\allowbreak{}cnao\_\allowbreak{}ring (EventAction/RunAction/SteppingAction) per classificare la dose depositata in ciascun neurone per TIPO di particella responsabile -\/- primario (parentID==0), elettrone secondario (gia\textquotesingle{} tracciato dalla sezione 5.39), o FRAMMENTO NUCLEARE SECONDARIO (qualunque altra traccia carica: protoni, alfa, deutoni, tritoni, ioni piu\textquotesingle{} pesanti). L\textquotesingle anello e\textquotesingle{} stato posizionato a picco+15mm per entrambe le particelle (49.5mm per il carbonio, 54.5mm per il protone), lo stesso margine relativo gia\textquotesingle{} usato nel confronto a fascio largo sopra.

Per il carbonio-12 (200.000 eventi): ZERO neuroni colpiti dal primario (fPrimaryHit sempre falso) -\/- conferma diretta che lo ione primario si e\textquotesingle{} fermato ben prima di questa profondita\textquotesingle, coerente col picco a 34.5mm. Eppure la dose totale sull\textquotesingle anello non e\textquotesingle{} nulla (0.99 MeV su tutti i 100 neuroni), e il 76.1\% di quella dose e\textquotesingle{} esplicitamente marcato come proveniente da frammenti nucleari secondari carichi -\/- la firma diretta, alla scala del singolo neurone, dello stesso fenomeno gia\textquotesingle{} visto a fascio largo.

Per il protone (stessa configurazione, 54.5mm): dose ESATTAMENTE zero su tutto l\textquotesingle anello, anche dopo 20.000.000 di eventi (oltre cento volte la statistica usata per il carbonio) -\/- un risultato negativo onesto, non un errore: e\textquotesingle{} quantitativamente coerente con la coda di frammentazione del protone gia\textquotesingle{} misurata \textasciitilde300 volte piu\textquotesingle{} debole di quella del carbonio a fascio largo, e con il limite di statistica per-neurone gia\textquotesingle{} documentato nelle sezioni 5.32 e 5.37 (li\textquotesingle{} anche la mappa di dose AL PICCO, molto piu\textquotesingle{} densa di questa coda residua, richiedeva centinaia di migliaia di eventi per pochi conteggi per neurone). L\textquotesingle effetto esiste ed e\textquotesingle{} misurabile a fascio largo, ma resta sotto la soglia di rilevabilita\textquotesingle{} alla scala del singolo neurone con una statistica computazionalmente realistica per questo progetto.

Sintesi: la coda di frammentazione del carbonio-12 e\textquotesingle{} verificata su due scale indipendenti (fascio largo e per-neurone) e due metodi indipendenti (profilo di dose totale e classificazione per specie di particella) che si rafforzano a vicenda invece di contraddirsi. E\textquotesingle{} un promemoria concreto, non solo teorico, del compromesso fisico reale alla base della scelta clinica tra ioni di carbonio e protoni -\/- lo stesso compromesso che ha motivato NIRS/Chiba a sviluppare i modelli RBE (Kanai, sezione 5.31) e Kase/MKM (sezioni 5.34-5.35) gia\textquotesingle{} usati in questo lavoro: un LET e un RBE piu\textquotesingle{} alti al bersaglio (il vantaggio del carbonio), a costo di una dose residua, per quanto piccola, ben oltre il volume bersaglio (il suo svantaggio) -\/- un aspetto che nessuna delle sezioni precedenti di questo filone aveva ancora reso misurabile direttamente.

\textit{\textbf{Codice sorgente: analysis/fragmentation\_\allowbreak{}tail.py}}

\lstinputlisting[language=Python]{geant4_src/fragmentation_tail.py}
\textit{\textbf{Codice sorgente: EventAction.cc / RunAction.cc / SteppingAction.cc (classificazione della dose per frammento nucleare secondario aggiunta in questa sezione)}}
...
\textit{\textbf{Codice sorgente: rete\_\allowbreak{}sintesi\_\allowbreak{}export.py (versione con esportazione dati 3D)}}
...
\item
  Il bias sistematico di posizione (\textasciitilde0.045 unita\textquotesingle) osservato ripetutamente negli esperimenti con il ring attractor (sezioni 4.4.2, 5.1, 5.2) non e\textquotesingle{} stato completamente diagnosticato alla radice -\/- probabilmente un artefatto di discretizzazione della griglia a N=200 punti, ma senza una verifica formale che lo escluda come effetto reale del modello {[}Aggiornamento: diagnosticato formalmente nella sezione 4.4.3 -\/- non e\textquotesingle{} un artefatto di discretizzazione (ne\textquotesingle{} spaziale ne\textquotesingle{} temporale), e\textquotesingle{} una proprieta\textquotesingle{} reale della dinamica del modello.{]}
\item
...
\bottomrule\noalign{}
\endlastfoot
due\_\allowbreak{}neuroni.py & 2.1 & Comunicazione base tra due neuroni \\
esplorazione\_\allowbreak{}parametri.py & 2.2 & Soglia sinaptica \\
plasticita\_\allowbreak{}sinaptica.py & 2.3 & Facilitazione a breve termine \\
rete\_\allowbreak{}catena.py & 2.4 & Propagazione in catena \\
rete\_\allowbreak{}convergente.py & 2.5 & Sommazione temporale \\
stdp.py & 2.6 & Spike-Timing-Dependent Plasticity \\
classificazione\_\allowbreak{}stdp.py & 2.7 & Apprendimento selettivo di pattern \\
rete\_\allowbreak{}300\_\allowbreak{}neuroni.py & 2.8 & Rete bilanciata E/I \\
rete\_\allowbreak{}300\_\allowbreak{}stimolo.py & 2.9 & Risposta a stimolo e recovery \\
due\_\allowbreak{}moduli.py & 2.10 & Architettura modulare \\
psth\_\allowbreak{}moduli.py & 2.11 & Verifica statistica su 25 prove \\
scaling\_\allowbreak{}verso\_\allowbreak{}topo.py, scaling\_\allowbreak{}fase2/3/4.py & 3.5 & Scaling progressivo automatico \\
rete\_\allowbreak{}40M\_\allowbreak{}finale.py & 3.6 & Esperimento finale a 40 milioni di neuroni \\
rete\_\allowbreak{}smallworld.py & 4.1 & Connettivita\textquotesingle{} small-world su anello \\
brain3d\_\allowbreak{}visualization.html, brain1M\_\allowbreak{}visualization.html & 4.2 & Visualizzazioni 3D interattive \\
ring\_\allowbreak{}attractor\_\allowbreak{}rate.py & 4.4 & Ring attractor, risultato finale verificato \\
diagnosi\_\allowbreak{}bias\_\allowbreak{}posizione\_\allowbreak{}scaling.py & 4.4.3 & Diagnosi del bias di posizione (N, sub-griglia, dt): non e\textquotesingle{} un artefatto numerico, e\textquotesingle{} una proprieta\textquotesingle{} reale del modello; scoperto e corretto anche un bug di riproducibilita\textquotesingle{} in ring\_\allowbreak{}attractor\_\allowbreak{}rate.py \\
chained\_\allowbreak{}attractors.py & 5.1 & Catena di due attrattori collegati \\
competing\_\allowbreak{}targets.py & 5.2 & Competizione tra alternative \\
rl\_\allowbreak{}bandit.py & 5.3 & Apprendimento per rinforzo (bandito) \\
contextual\_\allowbreak{}policy.py & 5.4 & Politica decisionale contestuale \\
rete\_\allowbreak{}sintesi\_\allowbreak{}export.py & 6.4-6.6 & Sintesi finale: spaziale + conduttanza + densita\textquotesingle{} Allen \\
dialogo\_\allowbreak{}ring\_\allowbreak{}attractor.py & 5.5.1 & Scambio a turni tra due ring, canali fissi \\
dialogo\_\allowbreak{}stdp.py & 5.5.2 & Sinapsi A-\textgreater B appresa (Hebbiano a grana di turno); asimmetria B-\textgreater A documentata \\
dialogo\_\allowbreak{}stdp\_\allowbreak{}adattamento.py & 5.5.3 & Adattamento + riscaldamento: entrambi i canali affidabili al 100\% \\
dialogo\_\allowbreak{}multi\_\allowbreak{}messaggio.py & 5.5.4 & Piu\textquotesingle{} messaggi sulla stessa sinapsi: test di interferenza \\
dialogo\_\allowbreak{}multi\_\allowbreak{}normalizzato.py & 5.5.5 & Normalizzazione omeostatica su W\_\allowbreak{}AB: competizione resa visibile, non risolta \\
dialogo\_\allowbreak{}multi\_\allowbreak{}repulsione.py & 5.5.6 & Esplorazione repulsiva: interferenza risolta, 7 scambi su 8 al 100\% \\
diagnosi\_\allowbreak{}bias\_\allowbreak{}posizione.py & 5.5.7 & Verifica diretta (e smentita) dell\textquotesingle ipotesi del bias di posizione \\
dialogo\_\allowbreak{}multi\_\allowbreak{}repulsione\_\allowbreak{}3msg.py & 5.5.8 & Scalabilita\textquotesingle{} a 3 messaggi: separazione migliore, consolidamento peggiore (risultato aperto) \\
dialogo\_\allowbreak{}multi\_\allowbreak{}scoperta\_\allowbreak{}locale.py & 5.5.9 & Repulsione solo per la scoperta + esplorazione locale: 10/12 scambi, miglior risultato \\
dialogo\_\allowbreak{}multi\_\allowbreak{}scoperta\_\allowbreak{}locale\_\allowbreak{}montecarlo.py & 5.5.10 & Verifica Monte Carlo (8 seed): 91.7\% di successo, ma l\textquotesingle ipotesi sul residuo era sbagliata \\
dialogo\_\allowbreak{}multi\_\allowbreak{}scoperta\_\allowbreak{}locale\_\allowbreak{}permutazione.py & 5.5.11 & Disambiguazione posizione/valore: turno 5 segue l\textquotesingle ordine, turno 7 segue il valore \\
dialogo\_\allowbreak{}multi\_\allowbreak{}scoperta\_\allowbreak{}locale\_\allowbreak{}mischiato.py & 5.5.12 & Ordine del round-robin mescolato: piu\textquotesingle{} equo tra i messaggi, ma tasso aggregato piu\textquotesingle{} basso (87.5\% vs 91.7\%) \\
dialogo\_\allowbreak{}multi\_\allowbreak{}scoperta\_\allowbreak{}bidirezionale.py & 5.5.13 & Scoperta+consolidamento locale su entrambi i canali: 95.8\%, turno 7 risolto al 100\% \\
ring\_\allowbreak{}attractor\_\allowbreak{}spiking.py & 5.6 & Primo ring attractor spiking stabile (10 tentativi di tuning, sinapsi NMDA-like) \\
dialogo\_\allowbreak{}spiking.py & 5.7 & Comunicazione appresa A-\textgreater B tra due ring attractor spiking, STDP vera \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale.py & 5.8 & Tentativo di bidirezionalita\textquotesingle{} spiking: stessa asimmetria del modello a rate, non risolta \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}turni.py & 5.9 & Schema a turni A/B per isolare il confondimento nel canale B-\textgreater A spiking \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}gate.py & 5.9 & Plasticita\textquotesingle{} B-\textgreater A confinata ai turni non confusi: conclusione negativa definitiva \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}multi.py & 5.10 & Due messaggi in dominio spiking, STDP vera: nessuna interferenza \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}multi\_\allowbreak{}montecarlo.py & 5.11 & Verifica Monte Carlo (8 seed): nessuna interferenza spiking confermata 16/16 \\
diagnosi\_\allowbreak{}sottosoglia\_\allowbreak{}ba.py & 5.12 & Verifica sotto-soglia del canale B-\textgreater A: confusa dalla dinamica propria di A \\
diagnosi\_\allowbreak{}ablazione\_\allowbreak{}ba.py & 5.12 & Ablazione: canale B-\textgreater A forte causa esplosione numerica, non segnale causale \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}silenzia.py & 5.12 & Silenziamento attivo di A: stessa esplosione, conclusione finale \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}interruttore.py & 5.13 & Interruttore di trasmissione: stabile con pesi bassi, un seed su quattro \\
diagnosi\_\allowbreak{}unidirezionale\_\allowbreak{}pesi\_\allowbreak{}forti.py & 5.13 & Controllo: canale unidirezionale con pesi forti e\textquotesingle{} stabile da solo \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}interruttore\_\allowbreak{}mc.py & 5.13 & Verifica Monte Carlo (4 seed): solo 1/4 stabile, la \textquotesingle vittoria\textquotesingle{} era un caso fortunato \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}omeostasi.py & 5.14 & Normalizzazione omeostatica dei pesi: bidirezionalita\textquotesingle{} spiking stabile sul seed di base \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}omeostasi\_\allowbreak{}mc.py & 5.14 & Verifica Monte Carlo (8 seed): 6/8 stabili, miglioramento reale ma non completo \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}omeostasi\_\allowbreak{}v2.py & 5.15 & Omeostasi piu\textquotesingle{} reattiva: crescita bidirezionale pulita sul seed di base \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}omeostasi\_\allowbreak{}v2\_\allowbreak{}mc.py & 5.15 & Verifica Monte Carlo (8 seed): 7/8 stabili, migliore di v1 \\
diagnosi\_\allowbreak{}seed301\_\allowbreak{}anello\_\allowbreak{}isolato.py & 5.16 & Verifica: anello isolato del seed 301 e\textquotesingle{} stabile da solo \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}omeostasi\_\allowbreak{}v3.py & 5.16 & Interruttore di emergenza sopra l\textquotesingle omeostasi graduale \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}omeostasi\_\allowbreak{}v3\_\allowbreak{}mc.py & 5.16 & Verifica Monte Carlo (8 seed): 7/8 puliti, ottavo migliorato 40x \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}omeostasi\_\allowbreak{}v4.py & 5.17 & Bidirezionalita\textquotesingle{} spiking risolta: interruttore trasmissione + omeostasi + soppressione diretta \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}omeostasi\_\allowbreak{}v4\_\allowbreak{}mc.py & 5.17 & Verifica Monte Carlo (8 seed): 8/8 stabili, crescita bidirezionale in ogni run \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}multi.py & 5.18 & Piu\textquotesingle{} messaggi bidirezionali spiking: completamento del filone \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}multi\_\allowbreak{}mc.py & 5.18 & Verifica Monte Carlo (8 seed): 8/8 stabili, nessuna interferenza \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}multi3.py & 5.19 & Tre messaggi bidirezionali spiking: stress-test del carico \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}bidirezionale\_\allowbreak{}multi3\_\allowbreak{}mc.py & 5.19 & Verifica Monte Carlo (8 seed): 8/8 stabili, 98\% localizzazione sotto soglia \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}relay\_\allowbreak{}3anelli.py & 5.20 & Staffetta a tre popolazioni A-\textgreater B-\textgreater C-\textgreater A, primo passo oltre la coppia \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}relay\_\allowbreak{}3anelli\_\allowbreak{}mc.py & 5.20 & Verifica Monte Carlo (6 seed): informazione preservata su due salti in ogni caso \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}knockout.py & 5.22 & Knockout dei neuroni a dose CNAO piu\textquotesingle{} alta: interrompe selettivamente il messaggio che li usa \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}knockout\_\allowbreak{}mc.py & 5.23 & Verifica Monte Carlo (8 seed): knockout robusto, 8/8 stessa direzione \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}knockout\_\allowbreak{}controllo\_\allowbreak{}mc.py & 5.24 & Controllo negativo: knockout casuale non riproduce l\textquotesingle effetto selettivo \\
...
dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}knockout\_\allowbreak{}v2\_\allowbreak{}mc.py & 5.26 & Ri-verifica con dose corretta: l\textquotesingle effetto si ribalta come previsto (test falsificabile superato) \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}knockout\_\allowbreak{}picco\_\allowbreak{}mc.py & 5.27 & Dataset completo al picco + gradiente dose-risposta su tre livelli di sovrapposizione \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}knockout\_\allowbreak{}proton\_\allowbreak{}mc.py & 5.28 & Knockout con mappa di dose da protoni: effetto piu\textquotesingle{} forte finora (7.07x, 8/8), gradiente a 4 punti completo \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}recupero.py & 5.29 & Recupero funzionale dopo il knockout: la plasticita\textquotesingle{} residua compensa progressivamente, verificato su 4 seed \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}lesione\_\allowbreak{}completa\_\allowbreak{}mc.py & 5.30 & Lesione completa della zona di codifica: il recupero visto in 5.29 sparisce, punto di non ritorno confermato su 5 run (1+4 MC) \\
geant4\_\allowbreak{}cnao\_\allowbreak{}ring (RunAction.cc + analisi RBE) & 5.31 & Modello RBE-LET (Kanai/NIRS): il gradiente 5.22-5.28 regge entro la stessa profondita\textquotesingle; scoperta variabilita\textquotesingle{} Monte Carlo tra run fisici non ancora verificata \\
geant4\_\allowbreak{}cnao\_\allowbreak{}ring (cnaoRingImpact.cc, seed esplicito) & 5.32 & Scoperto: la mappa di dose per-neurone e\textquotesingle{} dominata dal rumore Monte Carlo (overlap 1.0/10 su 4 seed); il meccanismo causale neuroscientifico resta valido, la corrispondenza a livello di singolo neurone no \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}knockout\_\allowbreak{}media4seed\_\allowbreak{}mc.py & 5.33 & Knockout da dose mediata su 4 seed Geant4: overlap 1-vs-0 indistinguibile dal controllo negativo, emerge una soglia minima di sbilanciamento (\textasciitilde2 neuroni) per un effetto reale \\
geant4\_\allowbreak{}cnao\_\allowbreak{}ring/analysis/mkm\_\allowbreak{}rbe.py & 5.34 & Modello MKM reale di Kase/NIRS (non piu\textquotesingle{} l\textquotesingle approssimazione lineare della 5.31), validato contro dati pubblicati entro il 10-15\%; conferma la 5.31 con un modello clinico vero \\
geant4\_\allowbreak{}cnao\_\allowbreak{}ring/analysis/mkm\_\allowbreak{}rbe.py (dataset completo) & 5.35 & MKM su tutte e sei le configurazioni CNAO: overlap fisica/biologica 9-10/10 ovunque; overkill del carbonio e RBE anomalo del protone a 150 MeV \\
geant4\_\allowbreak{}cnao\_\allowbreak{}ring/src/SteppingAction.cc (diagnostica CNAO\_\allowbreak{}DEBUG\_\allowbreak{}NEURON) & 5.36 & Verificato empiricamente (non solo ipotizzato) il picco di LET anomalo della 5.35: e\textquotesingle{} il protone primario quasi fermo (range straggling), non un frammento secondario -\/- collegato al dibattito reale sull\textquotesingle RBE variabile al bordo distale in protonterapia \\
build/results\_\allowbreak{}highstat/ (carbonio 5M eventi, seed 501/502) & 5.37 & Quantificato: 10x eventi migliora la sovrapposizione da 1.0/10 (puro rumore) a 2/10 -\/- non basta; confermato che mediare su seed (5.33) e\textquotesingle{} piu\textquotesingle{} efficiente che aumentare gli eventi \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}cnao\_\allowbreak{}knockout\_\allowbreak{}top5\_\allowbreak{}mc.py / top20\_\allowbreak{}mc.py & 5.38 & La dimensione del knockout (5, 10, 20 neuroni) non e\textquotesingle{} speciale: TOP-5 (overlap 0-vs-0) nullo come previsto, TOP-20 (overlap invertito) inverte l\textquotesingle effetto (1.74x, 7/8) esattamente come previsto \\
geant4\_\allowbreak{}cnao\_\allowbreak{}ring (DetectorConstruction.cc, taglio di produzione) & 5.39 & Primo passo del collegamento nanodosimetrico: scoperto e corretto un bug (taglio di produzione troppo grosso), spettro reale degli elettroni secondari verificato (100\% sotto 1 MeV, mediana 1.86 keV) -\/- il presupposto fisico dell\textquotesingle accoppiamento con Geant4-DNA e\textquotesingle{} confermato \\
geant4dna\_\allowbreak{}icsd/build/nanoICSD (energie reali dalla 5.39) & 5.40 & Collegamento nanodosimetrico completato: ICSD da energie di elettroni secondari REALI (CNAO) si inseriscono coerentemente tra i punti gia\textquotesingle{} pubblicati nel paper Villagrasa/Baiocco -\/- catena macro-a-nano verificata end-to-end \\
geant4dna\_\allowbreak{}icsd/nanoICSD.cc (seed esplicito, verifica 4 seed) & 5.41 & Verifica Monte Carlo di nanoICSD: M1=1.090 confermato stabile (CV 0.34\% su 4 seed) -\/- a differenza della mappa di dose (5.32), qui non c\textquotesingle e\textquotesingle{} un problema di rumore statistico \\
geant4dna\_\allowbreak{}ssbdsb (moleculardna, energie reali dalla 5.39) & 5.42 & Danno DNA reale (SSB/DSB) da elettroni secondari CNAO: frazione DSB piu\textquotesingle{} alta a bassa energia (LET piu\textquotesingle{} alto), verificato su 3 seed (SB CV 1.8\%, DSB CV 12.4\%) -\/- chiude il collegamento nanodosimetrico fino al livello del DNA \\
geant4dna\_\allowbreak{}ssbdsb/results/spectrum\_\allowbreak{}weighted (6 punti pesati) & 5.43 & Danno DNA pesato sull\textquotesingle intero spettro reale (6 punti, non 2): frazione DSB monotona con l\textquotesingle energia (15.7\% a 5.4\%), media pesata finale 9.5\% -\/- chiude l\textquotesingle ultimo limite dichiarato nella 5.42 \\
geant4\_\allowbreak{}cnao\_\allowbreak{}ring (tracking DSB per-neurone) & 5.44 & Ponte fisica-neuroscienza chiuso fino al DNA: classifica a danno DSB stimato per-neurone sovrapposta 9/10 con quella a dose fisica -\/- la dose fisica resta un criterio robusto anche al livello di realismo biologico piu\textquotesingle{} profondo raggiunto \\
dialogo\_\allowbreak{}spiking\_\allowbreak{}relay\_\allowbreak{}chiusura\_\allowbreak{}turni\_\allowbreak{}mc.py & 5.45 & Il loop C-\textgreater A non si chiude causalmente: verificato su 5 run (1+4 MC), mai piu\textquotesingle{} di 3/10 sonde con attivita\textquotesingle{} residua, sempre a livello di rumore -\/- estende la conclusione della 5.9 (B-\textgreater A non causale) al caso a tre anelli \\
geant4\_\allowbreak{}cnao\_\allowbreak{}ring/analysis/fragmentation\_\allowbreak{}tail.py & 5.46 & Coda di frammentazione del carbonio-12 oltre il picco di Bragg, confrontata col protone (fascio largo): carbonio \textasciitilde120-540x piu\textquotesingle{} dose residua a margini uguali \\
geant4\_\allowbreak{}cnao\_\allowbreak{}ring (EventAction.cc/RunAction.cc/SteppingAction.cc) & 5.46 & Classificazione della dose per-neurone per frammento nucleare secondario: 0 hit primari e 76.1\% dose da frammenti per il carbonio a picco+15mm; zero dose per il protone anche dopo 20M eventi \\
\end{longtable}
}
