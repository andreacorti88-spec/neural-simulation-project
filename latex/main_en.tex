...
\subsubsection{4.4.3 Formal diagnosis of the position bias: not a discretization artifact}\label{formal-diagnosis-of-the-position-bias-not-a-discretization-artifact}

Section 4.4.2 had observed a systematic bias of about 0.045 units between the stimulus position and the resulting bump position, hypothesizing -\/- without verifying it -\/- that it was "most likely a discretization artifact of the N=200-point spatial grid." Section 9 explicitly listed it as an open question: "determine whether it is a purely numerical artifact or reflects a real (if unwanted) property of the discretized model." This section answers with three independent, falsifiable tests.

A reproducibility bug discovered while preparing this diagnosis, fixed before proceeding: rerunning ring\_\allowbreak{}attractor\_\allowbreak{}rate.py to compare its output against what Section 4.4.2 documents, the file at the top level of the repository turned out to have been overwritten with a different implementation than the one actually validated -\/- a "Mexican hat" kernel (two opposing Gaussians, local excitation minus local inhibition) with a NON-saturating activation function, instead of the single excitatory kernel plus GLOBAL inhibition (proportional to the whole network\textquotesingle s mean) with a rectified activation saturating at r\_\allowbreak{}max=5.0 that was actually used to produce the four results verified in 4.4.2. With the wrong version, the bump always died out by the end of the simulation -\/- never the documented behavior. Comparing against the historical code embedded in the report itself (Section 4.4.2\textquotesingle s code table, frozen at the time of the original validation), it was possible to reconstruct and restore the version that actually works. Rerun, the corrected version NOW reproduces exactly the -0.045 bias at all 5 positions from 4.4.2 (saturation peak at 5.000 in each) -\/- confirming that the restored file is really the one behind the documented results, not just a plausible guess.

PART 1 -\/- scaling with grid resolution N. If the bias were an artifact of the Riemann sum approximating the continuous convolution integral (rec\_\allowbreak{}input = J\_\allowbreak{}exc @ r / N), it should shrink systematically as N grows, toward 1/N. The stimulus is deliberately placed at x=0.2537, a coordinate that NEVER exactly coincides with a grid node for any of the N values tested (50, 100, 200, 400, 800, 1600), so as not to confound the real scaling with a trivial snapping of the peak to the nearest grid node when N is small.

Result: the bias stays PRACTICALLY CONSTANT across the whole range (-0.034 at N=50, up to -0.046 at N=1600) -\/- a variation factor of just 0.74x between the two extremes, against the \textasciitilde32x REDUCTION factor a genuine 1/N quantization artifact should have produced going from N=50 to N=1600. If anything, the bias grows slightly with N instead of vanishing -\/- the exact opposite of the original hypothesis\textquotesingle s prediction.

PART 2 -\/- dependence on sub-grid position, at fixed N=200. If the bias depended on the stimulus position RELATIVE to the nearest grid node (the direct signature of a quantization-readout artifact), sweeping the stimulus continuously across a few grid cells should show a periodic structure of amplitude comparable to the bias itself.

Result: a periodic modulation with period EXACTLY 1/N does appear (the classic signature of quantizing the peak readout via argmax on a discrete grid) -\/- but at an amplitude much smaller than the main bias (a sawtooth between -0.0415 and -0.046, just 0.0045 peak-to-peak, one tenth of the \textasciitilde0.044 bias that Section 4.4.2\textquotesingle s hypothesis was trying to explain). A genuine discretization artifact does exist, then, but it is a secondary, negligible effect layered on top of a much larger bias that is substantially independent of sub-grid position.

PART 3 -\/- dependence on the time-integration step dt, at fixed N=200. If the bias were a truncation artifact of explicit Euler integration (a single Euler step\textquotesingle s local error scales linearly with dt), it should shrink proportionally as dt decreases.

Result: the bias is IDENTICAL to five decimal places (-0.04370) for every one of the four dt values tested (0.2, 0.1, 0.05, 0.025 ms, an 8x factor) -\/- no trace of dt-dependence, against the \textasciitilde8x reduction factor expected from a genuine truncation artifact linear in dt.

Conclusion: the ring attractor\textquotesingle s position bias is NOT a numerical artifact -\/- neither of spatial discretization (Part 1) nor of time integration (Part 3). It is a REAL property, however unwanted, of the model\textquotesingle s continuous dynamics. The most plausible hypothesis, not further verified in this work: an asymmetry in the TRANSIENT phase of bump formation, where the global inhibition term (proportional to the instantaneous mean of the ENTIRE network, which changes rapidly while the bump is forming) may not exactly preserve translational symmetry during the initial growth, before the system settles into its final steady-state shape -\/- a property of the dynamical trajectory, not of the equilibrium configuration itself. A periodic modulation of much smaller amplitude (one tenth of the main bias), with period exactly 1/N, is present and really is a quantization artifact of the peak readout -\/- but it is a secondary effect, not the explanation of the main phenomenon hypothesized in 4.4.2.

\includegraphics[width=6.3in,height=1.7071in]{media/media/image53.png}

\emph{Fig. 52 --- Diagnosis of the position bias: (left) bias vs. grid resolution N, practically constant against the 1/N scaling expected for an artifact; (center) bias vs. sub-grid position at N=200, showing a secondary periodic modulation of much smaller amplitude than the main bias; (right) bias vs. integration step dt, identical across an 8x factor.}

\textit{\textbf{Source code: diagnosi\_\allowbreak{}bias\_\allowbreak{}posizione\_\allowbreak{}scaling.py}}
...
\subsection{5.46 The carbon-12 fragmentation tail: protons and carbon compared beyond the peak}\label{the-carbon-12-fragmentation-tail-protons-and-carbon-compared-beyond-the-peak}

One aspect of carbon-12 physics never addressed in Sections 5.21-5.45, but central to the clinical literature and to the very history of Chiba/NIRS-HIMAC (the first center in the world to treat patients with carbon ions, since 1994, Section 5.31): unlike a proton, a carbon-12 ion passing through tissue can undergo inelastic nuclear reactions with target nuclei, fragmenting into lighter particles (protons, alpha particles, deuterons, tritons, or heavier isotopes such as lithium, beryllium, boron). These fragments, having a longer range than their parent ion at the same velocity, keep depositing dose EVEN BEYOND the primary beam\textquotesingle s Bragg peak -\/- the so-called "fragmentation tail," one of the most discussed physical trade-offs in the clinical comparison between carbon ions and protons, and one of the reasons why carbon\textquotesingle s higher RBE at the target (Sections 5.31-5.35) is not a cost-free advantage.

Method: braggProfile (a wide 5x5cm beam, 350 slices of 1mm, already validated in Section 5.25) was used to compare the depth-dose profile BEYOND the peak for the same two configurations already used throughout the CNAO thread -\/- carbon-12 at 120 MeV/u (peak at 34.5mm) and proton at 70 MeV (peak at 39.5mm) -\/- the energy choice from Section 5.28, to stay comparable with the rest of this work. The wide-beam geometry, already used to find the Bragg peaks in Section 5.25, gives vastly better statistics than cnaoRingImpact\textquotesingle s per-neuron geometry for an effect this rare (see below).

Result: carbon-12 shows a substantial, slowly decaying tail -\/- from 2.0\% of the peak at just 5mm beyond its own maximum, gradually falling to 0.05\% at 100mm, never dropping to zero. The proton, by contrast, shows an almost immediate collapse: already at 5mm beyond its own peak the residual dose has fallen to about 0.007\% of the maximum -\/- roughly 300 times less than carbon at the same relative distance -\/- and stays at that noisy level (limited statistics, consistent with the event being rare) for the entire depth tested, without carbon\textquotesingle s smooth, persistent tail. The ratio between the two tails, measured at several margins beyond each particle\textquotesingle s own peak (5, 10, 15, 20, 30, 50mm), ranges between 120x and 540x depending on the exact point (the proton\textquotesingle s statistics are noisy bin by bin, consistent with a rare event), but the qualitative conclusion is sharp and stable across the whole range: carbon\textquotesingle s fragmentation tail is a real physical effect, orders of magnitude larger than anything observable for a proton beam at the same relative energy.

\includegraphics[width=6in,height=4in]{media/media/image54.png}

\emph{Fig. 53 --- Fragmentation tail beyond the Bragg peak: dose relative to the peak (log scale) as a function of depth beyond each particle\textquotesingle s own peak, carbon-12 120 MeV/u vs. proton 70 MeV. Carbon shows a smooth, persistent tail; the proton collapses almost immediately to a noisy level \textasciitilde300x lower.}

Confirmation at the single-neuron scale: geant4\_\allowbreak{}cnao\_\allowbreak{}ring\textquotesingle s tracking (EventAction/RunAction/SteppingAction) was extended to classify the dose deposited in each neuron by the TYPE of particle responsible -\/- primary (parentID==0), secondary electron (already tracked since Section 5.39), or SECONDARY NUCLEAR FRAGMENT (any other charged track: protons, alphas, deuterons, tritons, heavier ions). The ring was positioned at peak+15mm for both particles (49.5mm for carbon, 54.5mm for the proton), the same relative margin already used in the wide-beam comparison above.

For carbon-12 (200,000 events): ZERO neurons hit by the primary (fPrimaryHit always false) -\/- direct confirmation that the primary ion stopped well before this depth, consistent with the peak at 34.5mm. Yet the total dose on the ring is not zero (0.99 MeV across all 100 neurons), and 76.1\% of that dose is explicitly tagged as coming from secondary charged nuclear fragments -\/- the direct, single-neuron-scale signature of the same phenomenon already seen at the wide-beam scale.

For the proton (same configuration, 54.5mm): EXACTLY zero dose across the entire ring, even after 20,000,000 events (over a hundred times the statistics used for carbon) -\/- an honest negative result, not an error: it is quantitatively consistent with the proton\textquotesingle s fragmentation tail already measured to be \textasciitilde300 times weaker than carbon\textquotesingle s at the wide-beam scale, and with the per-neuron statistics limitation already documented in Sections 5.32 and 5.37 (there, even the dose map AT THE PEAK, much denser than this residual tail, required hundreds of thousands of events for just a handful of counts per neuron). The effect exists and is measurable at the wide-beam scale, but stays below the detectability threshold at the single-neuron scale with a computationally realistic statistics budget for this project.

Synthesis: carbon-12\textquotesingle s fragmentation tail is verified across two independent scales (wide-beam and per-neuron) and two independent methods (total dose profile and particle-species classification) that reinforce rather than contradict each other. It is a concrete, not just theoretical, reminder of the real physical trade-off underlying the clinical choice between carbon ions and protons -\/- the same trade-off that motivated NIRS/Chiba to develop the RBE models (Kanai, Section 5.31) and Kase/MKM (Sections 5.34-5.35) already used in this work: a higher LET and RBE at the target (carbon\textquotesingle s advantage), at the cost of a residual, if small, dose well beyond the target volume (its disadvantage) -\/- an aspect none of this thread\textquotesingle s previous sections had yet made directly measurable.

\textit{\textbf{Source code: analysis/fragmentation\_\allowbreak{}tail.py}}

\lstinputlisting[language=Python]{geant4_src/fragmentation_tail.py}
\textit{\textbf{Source code: EventAction.cc / RunAction.cc / SteppingAction.cc (dose classification by secondary nuclear fragment added in this section)}}
...
\item
  The systematic position bias (\textasciitilde0.045 units) repeatedly observed in the ring-attractor experiments (Sections 4.4.2, 5.1, 5.2) has not been fully diagnosed at its root -\/- probably a discretization artifact of the N=200-point grid, but without a formal check ruling it out as a real effect of the model {[}Update: formally diagnosed in Section 4.4.3 -\/- it is not a discretization artifact (neither spatial nor temporal), it is a real property of the model\textquotesingle s dynamics.{]}
\item
...
\item
  Formally investigate the origin of the systematic position bias observed in the ring attractor (Sections 4.4.2, 5.1, 5.2), to determine whether it is a purely numerical artifact or reflects a real (if unwanted) property of the discretized model {[}Update: done in Section 4.4.3. Three falsifiable tests (scaling with N, sub-grid dependence, scaling with dt) rule out a numerical artifact: it is a real property of the model, however unwanted.{]}
\end{itemize}
...
diagnosi\_\allowbreak{}bias\_\allowbreak{}posizione\_\allowbreak{}scaling.py & 4.4.3 & Diagnosis of the position bias (N, sub-grid, dt): not a numerical artifact, it is a real property of the model; also discovered and fixed a reproducibility bug in ring\_\allowbreak{}attractor\_\allowbreak{}rate.py \\
...
geant4\_\allowbreak{}cnao\_\allowbreak{}ring/analysis/fragmentation\_\allowbreak{}tail.py & 5.46 & Carbon-12\textquotesingle s fragmentation tail beyond the Bragg peak, compared with the proton (wide beam): carbon \textasciitilde120-540x more residual dose at equal margins \\
geant4\_\allowbreak{}cnao\_\allowbreak{}ring (EventAction.cc/RunAction.cc/SteppingAction.cc) & 5.46 & Per-neuron dose classification by secondary nuclear fragment: 0 primary hits and 76.1\% fragment dose for carbon at peak+15mm; zero dose for the proton even after 20M events \\
\end{longtable}
}
