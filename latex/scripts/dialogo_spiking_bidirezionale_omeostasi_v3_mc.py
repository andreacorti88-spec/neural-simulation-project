"""
v3: aggiunge un interruttore di emergenza sopra la normalizzazione
omeostatica graduale della v2 (sezione 5.15). Il seed 301 esplodeva
identicamente in v1 e v2 nonostante tutto il resto fosse stabile --
verificato che non e' un problema di connettivita' ricorrente interna
(un anello isolato con la stessa connettivita' resta stabilissimo, solo
9 spike in 60s di rumore puro) ne' di grado di connessione inter-anello
anomalo (statistiche identiche agli altri seed). La correzione graduale
(al massimo +-1% ogni 20ms) evidentemente non basta a fermare
un'accensione che parte troppo in fretta per certe combinazioni di
rumore/tempismo. Qui si aggiunge un azzeramento IMMEDIATO (non graduale)
delle sinapsi in ingresso a un neurone se il suo tasso di scarica supera
una soglia critica (15Hz, ben sopra il bersaglio di 2.5Hz ma ben sotto un
vero runaway), come rete di sicurezza indipendente dalla velocita' di
reazione della correzione graduale.

Decimo tentativo sulla bidirezionalita' spiking. La sezione 5.13 ha
mostrato che l'interruttore di trasmissione (mai i due anelli connessi
bidirezionalmente nello stesso istante) non basta da solo: con pesi
conservativi il sistema resta sul bordo di un'instabilita' dinamica --
3 seed su 4 esplodono comunque. L'unico strumento usato finora per
contenere la crescita STDP era un tetto fisso (w_max) sul peso di
SINGOLA sinapsi: non impedisce che la somma di MOLTE sinapsi verso lo
stesso neurone cresca senza limite.

Tentativo qui: normalizzazione omeostatica (synaptic scaling, Turrigiano
1998) -- un meccanismo biologico reale, mai implementato altrove in
questo progetto, che regola non il peso di ogni singola sinapsi ma la
somma dell'input verso ciascun neurone, sulla base della sua stessa
attivita' recente. Ogni neurone E stima il proprio tasso di scarica
recente (una media mobile esponenziale, variabile "ratehat"); ogni 50ms,
tutte le sue sinapsi inter-anello IN INGRESSO vengono scalate
moltiplicativamente in modo da spingere quel tasso verso un bersaglio
fisso (target_rate). Se un neurone sta scaricando troppo, le sue sinapsi
in ingresso si indeboliscono automaticamente (indipendentemente da quale
STDP le abbia rinforzate); se scarica troppo poco, si rinforzano. A
differenza del tetto fisso, questo meccanismo reagisce alla vera
conseguenza funzionale (il tasso di scarica), non alla causa nominale
(il peso per sinapsi) -- l'ipotesi e' che possa impedire il collasso
anche quando i pesi individuali restano ben sotto al tetto.
"""
...
def omeostasi():              # margine di correzione nel tempo, reazione piu' rapida
    ...
    if emergenza_b.any():
        ...
        print(f"    [emergenza a t={defaultclock.t/ms:.0f}ms: {emergenza_b.sum()} neuroni di B azzerati, ratehat max={rate_b.max():.1f}Hz]")
    if emergenza_a.any():
        fattore_a[emergenza_a] = 0.0
        print(f"    [emergenza a t={defaultclock.t/ms:.0f}ms: {emergenza_a.sum()} neuroni di A azzerati, ratehat max={rate_a.max():.1f}Hz]")
...
print(f"\nTraining: {N_CICLI} turni alternati A/B/A/B/..., interruttore di "
      f"trasmissione + normalizzazione omeostatica (target {target_rate}) "
...
title(f'Raster completo: {N_CICLI} turni, interruttore + omeostasi (x={MESSAGGIO})')
...
tight_layout()
savefig('dialogo_spiking_bidirezionale_omeostasi_v3.png', dpi=150)
print("\nGrafico salvato in dialogo_spiking_bidirezionale_omeostasi_v3.png")
...
print("\n" + "="*70)
print("VALUTAZIONE: turni 'A' -- B risponde?")
print("="*70)
...
print("\n" + "="*70)
print("VALUTAZIONE: turni 'B' -- A risponde?")
print("="*70)
...
if n_a_tot > 200000 or n_b_tot > 200000:
    print("  >>> ESPLOSIONE: anche con la normalizzazione omeostatica il "
          "sistema e' instabile. <<<")
else:
    print("  >>> Stabile con la normalizzazione omeostatica attiva. <<<")

stabile = n_a_tot < 200000 and n_b_tot < 200000
(n_ab_primi, _), (n_ab_ultimi, _) = valuta_canale('B', mask_a, 5, escludi_diretto=True)
(n_ba_primi, _), (n_ba_ultimi, _) = valuta_canale('A', mask_b, 5, escludi_diretto=False)
print(f"MC_RIGA seed={SEED} stabile={int(stabile)} n_a_tot={n_a_tot} n_b_tot={n_b_tot} "
      ...
      f"b_da_a_primi={n_ba_primi} b_da_a_ultimi={n_ba_ultimi}")
