"""
Chiude il limite scoperto nella sezione 5.32: la mappa di dose per-
neurone di un SINGOLO run Geant4 e' dominata dal rumore di Poisson
(sovrapposizione 1.0/10 tra classifiche top-10 di 4 seed indipendenti
della stessa configurazione). Qui il knockout non viene piu' scelto da
un singolo run, ma dalla dose MEDIATA sui 4 seed (111, 222, 333, 444)
del carbonio-12 120 MeV/u al picco di Bragg -- la quantita'
statisticamente piu' solida disponibile finora per questo scenario.

Overlap con le zone di codifica (gruppo_stim, sezione 5.7): 0 neuroni
in x=0.15, 1 neurone (il 70) in x=0.65 -- uno sbilanciamento debole ma
reale. Secondo il gradiente dose-risposta gia' stabilito (5.28, dove
lo sbilanciamento correla con la forza dell'effetto), un overlap 1
contro 0 dovrebbe predire un effetto modesto ma nella direzione attesa
(x=0.65 piu' colpito di x=0.15) -- piu' debole del 2 contro 0 di 5.23,
ma non nullo come il ~1 contro ~1 di 5.27.
"""

KNOCKOUT = sorted([0, 5, 7, 29, 37, 47, 48, 70, 73, 82])  # top-10 per
                                                            # DOSE MEDIA sui 4 seed
                                                            # 111/222/333/444
                                                            # (carbonio-12 120 MeV/u,
                                                            # picco 34.5mm -- sezione 5.32)
...
tight_layout()
savefig('dialogo_spiking_cnao_knockout_media4seed.png', dpi=150)
print("\nGrafico salvato in dialogo_spiking_cnao_knockout_media4seed.png")
...
for k, m in enumerate(MESSAGGI):
    ...
    righe.append(f"m{k}(x={m}):n_i={n_i},n_f={n_f},err_f={err_f:.3f}")
print(f"MC_RIGA seed={SEED} " + " ".join(righe))
