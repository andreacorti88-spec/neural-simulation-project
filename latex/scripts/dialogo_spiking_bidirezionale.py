...
# --- canale di ritorno B->A, stessa architettura esatta di A->B --- ci si
# aspetta, per analogia con la sezione 5.5.3 del modello a rate, che A
# (un attrattore che una volta acceso non si spegne da solo, verificato in
# 5.6) sia difficile da "smuovere": B deve competere con un bump di A gia'
# a piena potenza, non accenderne uno da zero come A ha dovuto fare con B
syn_ba = Synapses(B_E, A_E, model=stdp_model, on_pre=stdp_on_pre, on_post=stdp_on_post)
mask_inter_ba = rng_inter.rand(N_E, N_E) < prob_inter  # stesso kernel, nuovo sorteggio
...
tight_layout()
savefig('dialogo_spiking_bidirezionale.png', dpi=150)
print("\nGrafico salvato in dialogo_spiking_bidirezionale.png")
...
def valuta_risposta_A_estesa(t0, t1):
    """Spike di A che arrivano DOPO la fine dell'impulso esterno (60ms
    dall'inizio del ciclo) in una data finestra di ripetizioni -- attivita'
    di A oltre quella spiegabile dal solo stimolo diretto, quindi
    potenzialmente dovuta al canale di ritorno B->A."""
    ciclo = (T_STIM+T_PAUSA)/ms
    fase = (spikes.t/ms - t0) % ciclo
    m = mask_a & (spikes.t/ms >= t0) & (spikes.t/ms < t1) & (fase > T_STIM/ms + 20)
    return m.sum()
...
print("\n" + "="*70)
print("VALUTAZIONE: il canale di ritorno B->A mostra segnale di apprendimento?")
print("="*70)
print(f"  Peso medio finale sinapsi B->A: {np.mean(syn_ba.w)/mV:.3f} mV (iniziale: {w_init/mV:.3f} mV)")
print(f"  Peso massimo B->A: {np.max(syn_ba.w)/mV:.3f} mV (iniziale: {w_init/mV:.3f} mV)")
a_extra_inizio = valuta_risposta_A_estesa(0, 5*(T_STIM+T_PAUSA)/ms)
a_extra_fine = valuta_risposta_A_estesa((N_RIPETIZIONI-5)*(T_STIM+T_PAUSA)/ms, N_RIPETIZIONI*(T_STIM+T_PAUSA)/ms)
print(f"  Spike di A oltre i 20ms dopo fine impulso esterno (possibile influenza di B):")
print(f"    prime 5 ripetizioni: {a_extra_inizio}   ultime 5 ripetizioni: {a_extra_fine}")
if np.mean(syn_ba.w)/mV > w_init/mV * 1.05:
    print("  >>> Il peso medio B->A e' cresciuto: la STDP sta trovando una "
          "correlazione pre/post reale su questo canale. <<<")
else:
    print("  >>> Il peso medio B->A e' rimasto sostanzialmente invariato: "
          "nessun segnale di apprendimento chiaro su questo canale, in "
          "questo run. <<<")
