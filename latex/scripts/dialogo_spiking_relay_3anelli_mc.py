...
syn_C = costruisci_anello(C_E, C_I, seed=2500+SEED)
...
syn_ab = collega(A_E, B_E, seed_conn=3010+SEED, w_iniziale=w_init)
syn_bc = collega(B_E, C_E, seed_conn=3011+SEED, w_iniziale=3.0*mV)
syn_ca = collega(C_E, A_E, seed_conn=3012+SEED, w_iniziale=3.0*mV)
...
print(f"\nTraining: {N_RIPETIZIONI} ripetizioni, stimolo SOLO su A, "
      f"nessuno stimolo diretto su B o C ({(T_STIM+T_PAUSA)*N_RIPETIZIONI/ms:.0f}ms totali)")
...
(n_c_i, c_c_i), (n_c_f, c_c_f) = valuta_pop(2, 5)
err_c_f = dist_circ(c_c_f - MESSAGGIO) if c_c_f is not None else 1.0
(n_b_i, c_b_i), (n_b_f, c_b_f) = valuta_pop(1, 5)
err_b_f = dist_circ(c_b_f - MESSAGGIO) if c_b_f is not None else 1.0
print(f"MC_RIGA seed={SEED} n_tot_rete={n_tot_rete} "
      f"b_n_i={n_b_i} b_n_f={n_b_f} b_err_f={err_b_f:.3f} "
      f"c_n_i={n_c_i} c_n_f={n_c_f} c_err_f={err_c_f:.3f}")
