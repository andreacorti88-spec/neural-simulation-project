"""
Verifica Monte Carlo di dialogo_multi_scoperta_locale.py: quella run aveva
dato 10/12 scambi riusciti, il migliore trovato, con un residuo (il terzo
messaggio, sempre l'ultimo del round-robin, non raggiunge le associazioni
di ordine piu' alto) attribuito a sfortuna nell'ordine di scoperta piuttosto
che a un limite strutturale nuovo -- ma un singolo seed casuale non basta a
dirlo con certezza (lo stesso principio statistico gia' applicato nel
progetto: PSTH su 25 prove indipendenti in sezione 2.11, rottura di
simmetria su 12 prove in 5.2.1).

Qui lo stesso esperimento (stessa configurazione, stesso codice, nessuna
modifica all'algoritmo) viene ripetuto con SEED diversi per rng_train --
l'unica vera fonte di casualita' (le posizioni esplorative scoperte). Se
l'ipotesi "sfortuna nell'ordine" e' corretta, il ruolo di "messaggio in
ritardo" dovrebbe ruotare tra i tre messaggi seed per seed, non cadere
sempre sullo stesso. Il seed si passa da riga di comando
(python3 dialogo_multi_scoperta_locale_montecarlo.py <seed>), cosi' le
repliche possono girare in parallelo invece che in sequenza.
...
if __name__ == '__main__':
    ...
    plt.tight_layout()
    plt.savefig(f'dialogo_multi_scoperta_locale_seed{SEED}.png', dpi=150)
    print(f"\nGrafico salvato in dialogo_multi_scoperta_locale_seed{SEED}.png")
