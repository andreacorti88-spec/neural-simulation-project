"""
Ultimo tentativo sulla bidirezionalita' spiking, dopo dialogo_spiking_
bidirezionale_turni.py: quello script ha isolato con precisione il
problema (0 spike di A durante i turni in cui SOLO B riceve stimolo
diretto), ma ha anche rivelato che il peso B->A continuava comunque a
crescere fino al tetto -- non grazie a quei turni puliti, ma per
coincidenza durante i turni 'A': B risponde all'eco di A cosi' in
fretta (decine di ms) che B e A finiscono per sparare quasi insieme
anche quando lo stimolo diretto e' solo su A, e la regola STDP sul
canale B->A non puo' distinguere le due situazioni.

Correzione chirurgica: invece di indebolire la dinamica A->B (che
comprometterebbe anche il canale che gia' funziona), si disattiva la
plasticita' STDP del solo canale B->A durante i turni 'A' (via una
variabile "shared" ltp_gate/ltd_gate, azzerata prima del turno A e
ripristinata prima del turno B). Il canale B->A puo' cosi' imparare
ESCLUSIVAMENTE nei turni in cui non esiste alcuna possibilita' di
coincidenza con lo stimolo diretto su A -- se cresce comunque, e' un
segnale causale pulito; se resta piatto, la conclusione negativa e'
definitiva e non ambigua.
"""
...
# --- canale A->B: STDP normale, sempre attiva, invariata rispetto ai
...
# --- canale B->A: stessa architettura, ma con ltp_gate/ltd_gate "shared"
# (un solo valore per l'intera Synapses, modificabile da fuori tra un
# run() e l'altro) al posto delle costanti A_ltp/A_ltd. Trasmissione
# (v_post += w) resta SEMPRE attiva -- solo l'aggiornamento del peso
# viene azzerato durante i turni 'A' ---
stdp_model_ba = '''
w : volt
dapre/dt = -apre/tau_stdp : volt (event-driven)
dapost/dt = -apost/tau_stdp : volt (event-driven)
...
N_CICLI = 60  # 30 turni 'A' + 30 turni 'B'
T_STIM = 60*ms
T_PAUSA = 940*ms

print(f"\nTraining: {N_CICLI} turni alternati A/B/A/B/..., plasticita' B->A "
      f"disattivata durante i turni 'A' ({(T_STIM+T_PAUSA)*N_CICLI/ms:.0f}ms totali)")
...
title(f'Raster completo: {N_CICLI} turni A/B, plasticita\' B->A gated (x={MESSAGGIO})')
...
tight_layout()
savefig('dialogo_spiking_bidirezionale_gate.png', dpi=150)
print("\nGrafico salvato in dialogo_spiking_bidirezionale_gate.png")

np.savez('dialogo_spiking_bidirezionale_gate_spikes.npz',
          t=np.asarray(spikes.t/ms), i=np.asarray(spikes.i),
          N_E=N_E, N_ring=N_ring, w_ba_final=np.asarray(syn_ba.w/mV),
          w_ba_i=np.asarray(syn_ba.i), w_ba_j=np.asarray(syn_ba.j))
print("Dati spike grezzi salvati in dialogo_spiking_bidirezionale_gate_spikes.npz")

ciclo_ms = (T_STIM+T_PAUSA)/ms


def valuta_canale(turni, lettera_turno, mask_dest, n_turni_eval, escludi_diretto):
...
print("\n" + "="*70)
print("VALUTAZIONE: turni 'A' -- B risponde? (canale A->B, sempre plastico)")
print("="*70)
(n_i, c_i), (n_f, c_f) = valuta_canale(turni, 'A', mask_b, 5, escludi_diretto=False)
...
print("\n" + "="*70)
print("VALUTAZIONE: turni 'B' -- A risponde? (canale B->A, plastico SOLO "
      "in questi turni)")
print("="*70)
(n_i, c_i), (n_f, c_f) = valuta_canale(turni, 'B', mask_a, 5, escludi_diretto=True)
print(f"  Primi 5 turni B: {n_i} spike di A (>20ms dopo inizio turno), centro={('N/A' if c_i is None else f'{c_i:.3f}')}")
print(f"  Ultimi 5 turni B: {n_f} spike di A (>20ms dopo inizio turno), centro={('N/A' if c_f is None else f'{c_f:.3f}')}")
print(f"  Peso medio B->A: {np.mean(syn_ba.w)/mV:.3f}mV (iniziale {w_init/mV:.3f}mV) max={np.max(syn_ba.w)/mV:.3f}mV")
if np.mean(syn_ba.w)/mV <= w_init/mV * 1.02:
    print("  >>> Con la plasticita' B->A confinata ai soli turni non-confusi, "
          "il peso NON cresce piu': la crescita vista nei tentativi "
          "precedenti era interamente un artefatto della coincidenza con i "
          "turni 'A'. Conclusione negativa pulita e definitiva. <<<")
elif n_f > n_i and n_f > 0:
    print("  >>> Il peso B->A cresce ANCHE isolato dai turni 'A', e A mostra "
          "attivita' crescente nei turni 'B': possibile segnale causale "
          "reale, da approfondire. <<<")
else:
    print("  >>> Il peso B->A si muove ma A non risponde in modo osservabile: "
          "risultato ambiguo. <<<")
