"""
Estensione di dialogo_multi_repulsione_3msg.py: la' due tentativi con
repulsione FISSA avevano mostrato una tensione -- repulsione debole (5)
lascia collidere le associazioni (9/12 scambi, ma una sola associazione
derivata domina la mappa dei pesi), repulsione forte (20) separa le tre
righe primarie ma respinge anche il rinforzo legittimo di un messaggio
dalla propria zona, impedendo il consolidamento (6/12, pesi crollati).

Qui la repulsione non e' piu' una costante ma decade nel tempo (annealing
esponenziale, funzione repulsione_corrente(ep)): forte all'inizio, quando
le zone vanno ancora rivendicate e separate; debole verso la fine, quando
serve lasciar crescere le righe gia' rivendicate senza ostacoli. L'idea:
separare temporalmente i due obiettivi in tensione (rivendicare / consolidare)
invece di bilanciarli con un unico valore fisso che finisce per accontentare
ne' l'uno ne' l'altro fino in fondo.
...
# budget
W_AB_COL_CAP = 1.5
# repulsione dell'esplorazione, ORA CON ANNEALING invece di un valore fisso.
# Due tentativi precedenti (dialogo_multi_repulsione_3msg.py) hanno mostrato
# una tensione: repulsione debole (5) lascia collidere le associazioni
# (9/12 scambi, ma una sola associazione derivata domina la mappa dei pesi);
# repulsione forte (20) separa correttamente le tre righe primarie ma
# respinge anche il rinforzo LEGITTIMO di un messaggio dalla propria zona,
# impedendo il consolidamento (6/12, pesi crollati). L'idea qui: i due
# obiettivi (rivendicare zone distinte / consolidarle) non devono avvenire
# con la STESSA intensita' di repulsione, ma in due fasi successive --
# repulsione alta all'inizio (quando le zone vanno ancora rivendicate),
# poi decadimento verso un valore basso (quando serve lasciar crescere le
# righe gia' rivendicate senza ostacoli). Decadimento esponenziale con
# costante di tempo TAU_DECADIMENTO_REPULSIONE.
REPULSIONE_INIZIALE = 20.0
REPULSIONE_FINALE = 3.0
TAU_DECADIMENTO_REPULSIONE = 2500.0


def repulsione_corrente(ep):
    return REPULSIONE_FINALE + (REPULSIONE_INIZIALE - REPULSIONE_FINALE) * np.exp(-ep / TAU_DECADIMENTO_REPULSIONE)
...
def campiona_esplorazione_repulsiva(rng, W, pos_propria, repulsione):
    ...
    La forza della repulsione e' passata dall'esterno (con annealing nel
    tempo), non piu' una costante fissa."""
    ...
    peso = 1.0 / (1.0 + repulsione * occupazione_altri)
    peso /= peso.sum()
    return rng.choice(x, p=peso)


def run_episode(W_AB, W_BA, messaggio, rng, train, allena_BA=True, repulsione=REPULSIONE_FINALE):
    rA = np.zeros(N)
    rB = np.zeros(N)
    aA = np.zeros(N)
    aB = np.zeros(N)
    ...
    for turn, listener in enumerate(SCHEDULE):
        ...
        if train and listener == 'B':
            explore_pos = campiona_esplorazione_repulsiva(rng, W_AB, messaggio, repulsione)
            ...
            explore_pos = campiona_esplorazione_repulsiva(rng, W_BA, pos_riferimento_B, repulsione)
            explore_profile = EXPLORATION_PULSE_STRENGTH * np.exp(
                -circ_dist(x-explore_pos)**2/(2*stim_width**2))
        else:
...
if __name__ == '__main__':
    ...
    for ep in range(N_EPISODES):
        ...
        repulsione = repulsione_corrente(ep)
        ...
        repulsione_storia.append(repulsione)
    ...
    ax_repulsione = axes[0, 1].twinx()
    ax_repulsione.plot(repulsione_storia, color='gray', linestyle='--', alpha=0.6, label='repulsione (annealing)')
    ax_repulsione.set_ylabel('Repulsione esplorazione', color='gray')
    lines1, labels1 = axes[0, 1].get_legend_handles_labels()
    lines2, labels2 = ax_repulsione.get_legend_handles_labels()
    ...
    plt.tight_layout()
    plt.savefig('dialogo_multi_repulsione_annealing.png', dpi=150)
    print("\nGrafico salvato in dialogo_multi_repulsione_annealing.png")
