"""
Seconda correzione tentata dopo 5.5.11-5.5.12. La'/qui il quadro e':
- turno 5 (canale B, torna ad ascoltare): effetto POSIZIONALE puro (5.5.11),
  un tentativo di correggerlo mescolando l'ordine del round-robin (5.5.12)
  ha reso il sistema piu' equo ma il totale aggregato e' sceso (87.5% da
  91.7%) -- non risolto, documentato come trade-off onesto.
- turno 7 (canale A, torna ad ascoltare): effetto legato al VALORE del
  messaggio (x=0.70 fragile all'87.5% ovunque si trovi nell'ordine),
  probabile connessione con il bias di posizione del ring attractor mai
  spiegato fino in fondo nel resto del progetto.

Osservazione strutturale che questo script corregge: il canale B->A (turni
3 e 7) non aveva MAI ricevuto il trattamento a due fasi (repulsione per la
scoperta, poi esplorazione locale per il consolidamento) introdotto in
dialogo_multi_scoperta_locale.py per il canale A->B -- restava con la sola
ricerca repulsiva per tutta la durata del training. Qui lo stesso
meccanismo (scoperta_A, analogo a scoperta_B) viene esteso anche al canale
di ritorno, per vedere se rende piu' stabili anche le associazioni di
ordine piu' alto sul lato A (turno 7 in particolare). Stesso protocollo a
8 seed (101-108) per la verifica di robustezza.
...
def run_episode(W_AB, W_BA, messaggio, rng, train, allena_BA=True, scoperta_B=None, scoperta_A=None):
    rA = np.zeros(N)
    rB = np.zeros(N)
    aA = np.zeros(N)
    aB = np.zeros(N)
    ...
    for turn, listener in enumerate(SCHEDULE):
        ...
        if train and listener == 'B':
            ...
            explore_pos = campiona_esplorazione(rng, W_BA, pos_riferimento_B, scoperta_A)
            explore_profile = EXPLORATION_PULSE_STRENGTH * np.exp(
                -circ_dist(x-explore_pos)**2/(2*stim_width**2))
        else:
...
if __name__ == '__main__':
    ...
    # come scoperta_B, ma per il canale di ritorno (W_BA, turni 3 e 7): non
    # aveva mai avuto la fase di consolidamento locale finora (solo la
    # ricerca repulsiva), probabile causa per cui il turno 7 restava piu'
    # fragile del turno 3 -- qui riceve lo stesso trattamento in due fasi
    scoperta_A = {m: None for m in MESSAGGI}
    ...
    for ep in range(N_EPISODES):
        ...
        pos_A, pos_B = run_episode(W_AB, W_BA, messaggio, rng_train, train=True,
                                    ...
                                    scoperta_A=scoperta_A[messaggio])
        if pos_B[1] is not None:
            scoperta_B[messaggio] = pos_B[1]
        if pos_A[3] is not None:
            scoperta_A[messaggio] = pos_A[3]
        if pos_A[7] is not None:
            scoperta_A[messaggio] = pos_A[7]
    ...
    plt.tight_layout()
    plt.savefig(f'dialogo_multi_scoperta_bidirezionale_seed{SEED}.png', dpi=150)
    print(f"\nGrafico salvato in dialogo_multi_scoperta_bidirezionale_seed{SEED}.png")
